...
| `student_placements` | `UNIQUE(student_id)` — one active placement per student; keep an append-only history table. **⚠️ NOT AUTHORITATIVE — superseded 2026-09-21 by O-01 final decision (kept as historical sketch, not deleted):** this line predates the placement model and must **not** be used as the schema form. The decided key per O-01 (per-program, 2026-09-21) is **`UNIQUE(student_id, program_id)`** — one active placement per **`(student, program)`**, consistent with `src/domains/learning/types.ts`. A teacher may grant a specific student exceptional access to a specific resource outside normal level eligibility (resource-grant only; does not change level/program placement); the persistence model for that exception is a future decision, not designed here. See `docs/frontend-completion/12-decision-register.md` → “O-01 closure — 2026-09-21”. |
