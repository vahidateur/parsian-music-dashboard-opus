...
| **Documentation checkpoint (pushed)** | `98af31b5b04022362f0bc95ca99c426f2d7c8446` — **the D5 finalization record** (*docs(governance): finalize decision D5 — four-category model and seventeen owner clauses*): the separation shape M10 executed was recorded and finalized, the earlier governance passes' stale claims were corrected (L3 scope, the "third role already unused" premise, the fixture line/import census), and **L2** plus the five-name drift table were left scoped for M10. Documents only; no product source, no test, no dependency, no gate. **The twenty-ninth** documentation checkpoint *by this ledger's count — the previously registered one is the twenty-eighth `3a16549` —* which could not name itself — this M10 closure pass names it — and **the commit M10 was built on** |
| Previous documentation checkpoint | `3a16549989f6ac41ba550e18107c62ec896f32a5` — **M9's documentation closure** (*docs(m9): close the dashboard insight phase and register the M9 checkpoint*): M9 registered ✅ **COMPLETE** against its measured evidence, with the milestone's limitations kept recorded rather than smoothed over. Documents only; no product source, no test, no dependency, no gate. **The twenty-eighth** documentation checkpoint, which could not name itself — the D5 record named it — and the commit **the D5 record is built on** |
...
- A **documentation checkpoint** is a pushed commit that changes documents and validation gates
  but no product behaviour. **Twenty-nine** are **named** so far — the thirtieth is the commit carrying
