...
| Field | Value |
|---|---|
| Working branch | `arena/01a0aa83-parsian-music-dashboard-opus` — the actual M10 implementation branch, carrying implementation commit `e7a64b7777be36ddd97fc3337898fad794118e5b` on top of parent `98af31b5b04022362f0bc95ca99c426f2d7c8446`, and now also the M11 implementation chain `3eed4f190b6538ad62bf35d7b0c9fa5afc310acc` → `24998d8330a95510cd969c872edb28ddcc59ef16` → `53794d57a7375c8345f76adec9248f5da09d7a92` → `f7eb86762dc11f7d5dd3c7218e7c697fad29488e` on top of the D7/D8/D9 decision record `3ed0bfe4ac928451a4bddf9de3a34f7be681c33f`. **Branch-name errata (documented, not repaired):** the name an earlier M10 note designated — arena/01a0a9b6-parsian-music-dashboard-opus — **does not exist** in this repository (verified against origin and every local ref) and was never touched; no rename, merge, cherry-pick, reset, rebase, force-push or other branch repair was performed. |
...
| Field | Value |
|---|---|
| Current phase | **Product-feature phase — M11 (performance I6, api-hybrid disclosure D8, accessibility D7, browser-QA checklist)** |
| Phase status | ✅ **COMPLETE** — **M11** (performance **I6**, api-hybrid disclosure **D8**, accessibility **D7**, browser QA **L4**) was implemented across **four** checkpoints, `3eed4f190b6538ad62bf35d7b0c9fa5afc310acc` (*perf(m11): I6 — code-split views, split vendor chunk, prune dead fonts*), `24998d8330a95510cd969c872edb28ddcc59ef16` (*test(m11): encode the D9 bundle budget gate on measured values*), `53794d57a7375c8345f76adec9248f5da09d7a92` (*feat(m11): D8 — declare demo-backed domains in api mode*) and `f7eb86762dc11f7d5dd3c7218e7c697fad29488e` (*test(m11): D7 — dependency-free accessibility gate*), **explicitly authorized by the owner** for the exact scope on 2026-09-17 and built on `3ed0bfe4ac928451a4bddf9de3a34f7be681c33f` — the D7/D8/D9 decision record, the thirty-first documentation checkpoint by this ledger's count, which is therefore M11's **effective safe rollback boundary** — and this documentation pass reconciles the record. **Starting SHA = `3ed0bfe`; final implementation HEAD = `f7eb867`** (an implementation-report wording that named `f7eb867` the starting SHA is corrected). **Performance:** every routed view except first-paint Login/Dashboard lazy-loads in two workspace groups (`src/views/lazy/` re-export barrels), the React family rides a hashed **vendor** chunk, `chunkSizeWarningLimit` is deliberately unset (**D9**), and Vazirmatn **300/800** are gone. **Final D9 measurements** (CLI `gzip -6 -n`, baseline method): entry JS gzip **108 207 B** (was 273 791 B), vendor gzip **60 108 B**, total JS+CSS+HTML gzip **298 249 B** (max single chunk 108 207 B; none ≥ 400 000 B), `dist/` raw **1 964 789 B**, the >500 kB raw warning cleared by real splitting. **D8:** the persistent, non-dismissable `DemoBackedNotice` derives its wording from the single `DEMO_SERVED_DOMAINS` enumeration at the composition root, renders in api mode, is absent in demo mode, and claims nothing about backend health. **D7:** fourteen dependency-free a11y gate cases — role/name identity, focus-trap entry/loop/escape/return, command-palette keyboard flow, `prefers-reduced-motion`, RTL/Persian invariants. **Protected:** I2/L7/backend/Compensation/Attendance/persistence/CSP/M10 boundary untouched; no dependency change; M10's chain intact. **L4 stays NOT VERIFIED** — no browser tooling exists or was installed; the external-QA disposition stands (§5). **Accepted with recorded limitations** — §4 → "M11 validation" and §7 item 23; **browser QA NOT VERIFIED** (§5). |
...
**M10 and M11 are now both ✅ COMPLETE (M10 on 2026-09-16 at `e7a64b7777be36ddd97fc3337898fad794118e5b`;
M11 on 2026-09-17, four checkpoints ending at `f7eb86762dc11f7d5dd3c7218e7c697fad29488e`) — and the phase
specification names no milestone beyond M11, so the next phase is NOT STARTED and NOT AUTHORIZED.** The
