...
>
> **Live governance law:** [GOVERNANCE_CHECKPOINT.md](GOVERNANCE_CHECKPOINT.md) — the 2026-09-20
> register for `T-02` / `O-*` / `PERF` / `HELP`; reached from [DECISIONS.md](DECISIONS.md) **§20**,
> which is a *pointer* and amends no D-entry.
> **Consolidation provenance:** [PRE_CLEANUP_HANDOFF.md](PRE_CLEANUP_HANDOFF.md).
> **Current session handoff:** [SESSION_HANDOFF.md](SESSION_HANDOFF.md).
> **Historical, superseded — do not treat as current law:**
> `docs/frontend-completion/13-open-decisions.md` (the F0 / 2026-09-19 snapshot) and
> [archive/SESSION_HANDOFF-2026-09-18-pre-M1.md](archive/SESSION_HANDOFF-2026-09-18-pre-M1.md)
> (the pre-M1 handoff, preserved unedited).
>
> **Frontend freeze closure — 2026-09-19 — documentation-only — frozen HEAD `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb` on canonical `arena/01a0b6be-parsian-music-dashboard-opus`.** Arena frontend phase formally closed at this checkpoint. **M-1 Quick Actions CLOSED**, **D7 Accessibility CLOSED**, **M-2 Teacher Photo CLOSED**, **M-3 Student Photo CLOSED**, **M-4 I15 Failure Disclosure CLOSED**, **M-5 I16 Deep Links CLOSED**, **M-6 Settings Honesty CLOSED**, **D9 Performance CLOSED** — measured at `a3867a6`: entry CSS gzip 16.39kB, vendor 60.29kB, index 118.50kB, operations 47.07kB, academic 47.86kB, total JS+CSS+HTML ~300kB within +5% cap 305805 B, no chunk ≥400kB, 2037 modules, `npx tsc --noEmit --skipLibCheck` 0 errors, focused 92 tests pass (settingsHonesty 14, deepLinkPagination 11, failureDisclosure 24, metricsFailureDisclosure 15, commandPaletteFocusReturn 4, teacherPhoto 8, studentPhotoDisplay 6, navigationCounts 10), full suite 2102 passed / 10 failed — 10 failures are **known historical** `src/__tests__/projectState.test.ts` SHA/branch expectation failures, not regressions, test not altered. **Browser QA NOT VERIFIED** as formal final-audit disposition, no claim of complete browser verification, no claim that backend functionality already exists. **Deferred:** Classes deep-link beyond capped list, Finance/Reports backend/domain implementation, notification server wiring, localization/settings server wiring, working-hours/session-rules server wiring, free-slot search, external browser QA — **deferred items do NOT modify the frozen frontend**. **Backend boundary:** frontend freeze precedes Laravel/backend implementation. **Canonical branch policy:** all future frontend fixes require explicit decision to reopen freeze. Historical checkpoints preserved: M11 chain `3ed0bfe→3eed4f1→24998d8→53794d5→f7eb867`, M10 `e7a64b7` on `98af31b`, previous working branch `arena/01a0aa83` now superseded by canonical `arena/01a0b6be-parsian-music-dashboard-opus`, other branches unchanged (main `a646975` — *superseded 2026-09-20: `main` advanced to `f63ebaab37096fcb25063aca51c713e26d3b3da7` by the PR #5 merge, `a646975` remaining its previous tip and an ancestor*, `arena/01a0aa83` `d31cbe9`, `arena/01a0b059` `251af96`, `handoff/...` `94d32de`).

> **✅ CONSOLIDATION COMPLETE — 2026-09-21 — the canonical branch is `main` @ `f63ebaab37096fcb25063aca51c713e26d3b3da7`.** PR #5 (*Consolidate frontend F0–F10 + governance into main*) was **MERGED** on 2026-09-20T21:11:18Z, so `main` now carries the whole line: the Arena Frontend Freeze work (M-1..M-6, D7, D9), the M10 `e7a64b7` on `98af31b` and M11 `3ed0bfe→3eed4f1→24998d8→53794d5→f7eb867` chains, the **F0–F10 frontend product completion** (`docs/frontend-completion/` 01–17 + README) and the governance checkpoint. **PR #4** (*docs: frontend product completion architecture + spec + execution map*) is **CLOSED and was NEVER MERGED** — its `f00f85e6f817369a32b8921ffb64f58e17ea154e` line reached canonical history through PR #5's merge commit `112f1160104d375481b930517d08592bb67a0a72` instead. **Because `main` advanced by merge only, no history was rewritten:** `a6469759fea743b9332702bb9a20e69fc4abc1be` is still `main`'s previous tip and remains an **ancestor** of `f63ebaab`. **Four historical SHAs preserved and reachable:** `f00f85e6f817369a32b8921ffb64f58e17ea154e` (branch `arena/frontend-completion-spec` + `refs/pull/4/head`, **PR #4 head**), `02b74996e6458d10a5d9e8d1023890239342c4e4` (the freeze ancestor, now an ancestor of `main`; branch `arena/01a0b6be-parsian-music-dashboard-opus`), `112f1160104d375481b930517d08592bb67a0a72` (branch `arena/01a0bf6d-parsian-music-dashboard-opus` + `refs/pull/5/head`, **PR #5 head**), `59fdacf9606fdc2ded3f060603405f94c7408d30` (**in `main`'s own first-parent history** — `…7a44c6a → 59fdacf9 → 112f1160 → f63ebaab`; it has no ref tip of its own, which is exactly why it is recorded here). **Live governance law is [GOVERNANCE_CHECKPOINT.md](GOVERNANCE_CHECKPOINT.md)** (recorded 2026-09-20; the T-02 / O-* / PERF / HELP register), reached from [DECISIONS.md](DECISIONS.md) **§20**, which is a *pointer* and amends no D-entry. **`T-02` is NOT unresolved:** its live status is **ACCEPTED / ASSIGNED-ONLY** — teacher student-read = active enrollment ∩ `class.teacherId`, `Student.teacherId` is not authorization, fail-closed, do not revisit. Any text below that calls `T-02` *OPEN / EVIDENCE CONFLICTING* is the superseded F0 / 2026-09-19 snapshot, preserved only as history. **Historical — do NOT treat as current law:** `docs/frontend-completion/13-open-decisions.md` (the F0 / 2026-09-19 snapshot; its `O-01..O-20` **and** `T-02` statuses are superseded by the 2026-09-20 checkpoint) and [archive/SESSION_HANDOFF-2026-09-18-pre-M1.md](archive/SESSION_HANDOFF-2026-09-18-pre-M1.md) (the pre-M1 handoff, preserved unedited). **Consolidation provenance:** [PRE_CLEANUP_HANDOFF.md](PRE_CLEANUP_HANDOFF.md) §1/§8/§9, whose "not yet safe" verdict it now records as discharged. **Laravel/backend remains ❌ NOT STARTED and NOT AUTHORIZED** — the Git-consolidation gate that preceded it is discharged, but that is not an authorization to begin backend work. **D1 documentation checkpoint now sits on top — `main` @ `6ca25be654b56579054c88cd498a42f57ea63582`** (the docs-only D1 pass, recorded in §2 below): `f63ebaab` remains the **consolidation merge itself**, **D1 is not that merge**, and **D2 is the pass being created now** — which cannot name itself.

> **⛔ HISTORICAL RECORD — 2026-09-19 — preserved, NOT current state.** The `CURRENT_BRANCH` / `CURRENT_HEAD` / `PLANNING_CHECKPOINT` values inside this blockquote and the F1-KICKOFF blockquote below it are the values **at that date**, on the pre-consolidation branch `arena/frontend-completion-spec`. Today's canonical values are the ones in the consolidation blockquote above and in §2. This region is kept for audit.

> **Frontend freeze INTENTIONALLY REOPENED — 2026-09-19 — FRONTEND PRODUCT COMPLETION — F1-KICKOFF DOCUMENTATION CORRECTION — branch `arena/frontend-completion-spec` — CURRENT_BRANCH: `arena/frontend-completion-spec` — CURRENT_HEAD: `a251e6942b2a645987286de6e822607554e84df9` = `a251e69` (actual Git HEAD before this F1-KICKOFF correction, frozen `a3867a6` descendant, 11 commits ahead of `02b7499`) — PLANNING_CHECKPOINT: `e57bf1922e4ca3b80796715028445cfe8a077c9c` = `e57bf19` (last completed planning checkpoint before documentation-only correction commits) — PREVIOUS_PLANNING_CHECKPOINT: `c244fec142fcc35faff8e8548c8ebbc05cb53bf5` = `c244fec` preserved as historical — PREVIOUS_CORRECTION_COMMITS: `969509783748b18b04eb17ca759127e420a3784c` = `9695097` docs-only stale SHA fix NOT planning milestone + `a251e6942b2a645987286de6e822607554e84df9` = `a251e69` SHA SEMANTICS ONLY NOT planning milestone — BEFORE = `a251e6942b2a645987286de6e822607554e84df9` = `a251e69` (actual HEAD before this F1-KICKOFF correction) — AFTER = new correction commit (to be — this commit) documentation-only F1-KICKOFF CORRECTION NOT planning milestone — PLANNING_CHECKPOINT = e57bf19 remains last completed planning checkpoint — CURRENT_HEAD = a251e69 is actual Git HEAD before this correction — SHA semantics no longer ambiguous after this correction: BEFORE = a251e69, AFTER = new commit, PLANNING = e57bf19, PREVIOUS = c244fec — do NOT call e57bf19 the current Git HEAD — PR #4 https://github.com/vahidateur/parsian-music-dashboard-opus/pull/4 docs-only OPEN — spec `docs/frontend-completion/` v14 @ `e57bf19` planning checkpoint + corrections + F1-KICKOFF — F0 Documentation/state closure COMPLETE + REVIEW CORRECTIONS DONE + FINAL CHECKPOINT CORRECTION stale SHA fix DONE + SHA SEMANTICS CORRECTION DONE + F1-KICKOFF CORRECTION — reusable DoD 15 items — F1 dispositions Library detail view same surface / publication status active/visibility deferred workflow / inactive-archived OPEN / gallery seed 2 albums 0 images VERIFIED — C-2 Telegram Backup REQUIRED preserved — C-4 BEFORE a251e69 AFTER new commit PLANNING e57bf19 PREVIOUS c244fec — T-02 OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION — NEXT ACTION exactly ONE F1 — F1 NEXT — exactly ONE NEXT ACTION F1.** Historical freeze at `a3867a6` preserved above, not deleted. Freeze reopened by explicit decision for Frontend Product Completion Architecture + Spec + Execution Map. **CURRENT_BRANCH:** `arena/frontend-completion-spec`. **CURRENT_HEAD:** `a251e6942b2a645987286de6e822607554e84df9` = `a251e69` (actual Git HEAD before this F1-KICKOFF correction, 11 commits ahead of `02b7499`, previous planning checkpoint `c244fec` preserved as historical, previous correction commits `9695097` + `a251e69` documentation-only NOT planning milestones). **PLANNING_CHECKPOINT:** `e57bf1922e4ca3b80796715028445cfe8a077c9c` = `e57bf19` (last completed planning checkpoint before documentation-only correction commits). **PREVIOUS_PLANNING_CHECKPOINT:** `c244fec142fcc35faff8e8548c8ebbc05cb53bf5` = `c244fec` preserved as historical. **PREVIOUS_CORRECTION_COMMITS:** `9695097` docs-only stale SHA fix + `a251e69` SHA SEMANTICS ONLY documentation-only, NOT planning milestones. **BEFORE:** `a251e6942b2a645987286de6e822607554e84df9` = `a251e69` (actual HEAD before this F1-KICKOFF correction). **AFTER:** new correction commit (to be — this commit) documentation-only F1-KICKOFF CORRECTION NOT planning milestone — PLANNING_CHECKPOINT remains e57bf19. **Frontend completion spec path/version:** `docs/frontend-completion/` v14 @ `e57bf19` planning checkpoint + corrections + F1-KICKOFF — 01-audit-reconstruction, 02-capability-matrix, 03-domain-map, 04-learning-access-policy, 05-rbac-access-control (T-02 OPEN EVIDENCE CONFLICTING), 06-library-gallery-spec (gallery seed 2 albums 0 images VERIFIED + F1 dispositions), 07-settings-theme-architecture, 08-export-architecture, 09-dashboard-analytics, 10-integration-architecture (C-2 Telegram Backup REQUIRED preserved), 11-roadmap (reusable DoD 15 items + F1 dispositions), 12-decision-register, 13-open-decisions, 14-handoff-checkpoint + README — previous `c244fec` remains as historical checkpoint, `e57bf19` remains planning checkpoint, `a251e69` is actual HEAD before this correction. **PR #4:** https://github.com/vahidateur/parsian-music-dashboard-opus/pull/4 — docs-only, base `arena/01a0b6be-parsian-music-dashboard-opus`, head `arena/frontend-completion-spec`, state OPEN not merged — do NOT merge — body refreshed. **Current phase:** FRONTEND_COMPLETION — **F0 Documentation/state closure COMPLETE + REVIEW CORRECTIONS DONE + FINAL CHECKPOINT CORRECTION stale SHA fix DONE + SHA SEMANTICS CORRECTION DONE + F1-KICKOFF CORRECTION DONE** — 14 docs committed + corrections + F1-KICKOFF, classification A/B/C evidence-based, capability matrix, domain map, learning policy, RBAC testable contract 11 cases with T-02 explicitly OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION CURRENT OBSERVED BEHAVIOR DENIED PRODUCT DECISION OPEN, library/gallery with F1 dispositions, settings/theme, export, dashboard, integration REQUIRED classification, roadmap F0..F10 canonical with reusable DoD 15 items + F1 dispositions, decision register, open decisions, handoff — SHA semantics no longer ambiguous — BEFORE a251e69 AFTER new commit PLANNING e57bf19 PREVIOUS c244fec. **Completed planning checkpoint:** F0 docs/state closure DONE + REVIEW CORRECTIONS docs-only pass DONE — PLANNING_CHECKPOINT e57bf19 exactly, PREVIOUS_PLANNING_CHECKPOINT c244fec historical only, CURRENT_HEAD a251e69 actual Git HEAD before this F1-KICKOFF correction, PREVIOUS_CORRECTION_COMMITS 9695097 + a251e69 documentation-only NOT planning milestones — BEFORE a251e69 AFTER new commit PLANNING e57bf19 PREVIOUS c244fec — safety gate docs-only passes, no src/tests/pkg/backend changes. **Accepted decisions:** D1 student role deferred separate app, D2 viewer vs org split appearance device-local branding org, D5 one owner per rule/source per concept, D6 finance/reports deferred no fabricated, D7 nav hints without numbers, D8 demo served both modes 11 domains disclosure, D9 bundle budget, D10 attach intent guard programId independent, D11 derive selection, D12 failed secondary read reported as failure, D13 wired view derives own selection + register, D14 no new verb chat export, D15 media resolution != auth, D16 conversation-keyed write, D17 relation bounded read count claim needs complete, D18 compensation obligation entity derived state append-only ledger, D19 compensation one-to-one only registered by person ordinary session via scheduling repo, D20 compensation eligibility disclosed never enforced, B1 Laravel domain structure Sanctum cookie primary + bearer fallback, NEW-LRN-01 eligibility canonical Level N=>1..N owner learning/eligibility.ts, NEW-LIB-01 library model, NEW-GAL-01 gallery genuine, NEW-SET-01 settings/theme tokens WordPress-customizer-like, NEW-EXP-01 export reusable, NEW-DASH-01 dashboard no fabricated Raw->Derivation->Insight->Visualization->Export, NEW-RBAC-01 preserve vocab smallest, NEW-PORTAL-01 student portal architecture, NEW-CHAT-01 tickets/chat/files topology, NEW-TG-01 Telegram backup vs access adapters not business owner, NEW-BALE-01 Bale avoid duplicate logic Core->Adapter, NEW-MOB-01 mobile same backend, NEW-CLASS-01 classification A/B/C evidence-based + 3 agent proposals PROP-BRAND-01 draft preview, PROP-LIB-01 level string vs relation, PROP-THEME-01 viewer vs org accent naming + F1-KICKOFF dispositions Library detail view same surface / publication status active/visibility deferred workflow / inactive-archived OPEN / gallery seed 2 albums 0 images VERIFIED — do NOT change product decisions — reusable DoD 15 items. **Open decisions:** 20 O-01..O-20 — O-01 student level scope per-program vs global vs per-instrument OPEN, O-02 level/resource relation separate vs merged, O-03 visibility library/gallery, O-04 theme persistence org vs device-local, O-05 export formats, O-06 permission analytical export, O-07 ticket vs chat ownership scope, O-08 file ownership owner+org_id, O-09 Telegram backup semantics REQUIRED, O-10 identity linking user↔student self/guardian, O-11 Bale linking REQUIRED, O-12 mobile auth bearer vs cookie REQUIRED, O-13 student portal auth separate app REQUIRED, O-14 org/user relation org_id all tables, O-15 API boundaries envelope binary streaming cursor linking, O-16 media storage S3 signed scanning, O-17 notification provider integration REQUIRED, O-18 audit general vs per-domain, O-19 retention, O-20 backup restore versioned integrity encryption REQUIRED — high-cost 7 before Laravel schema/API O-01,O-02,O-08,O-09,O-10,O-13,O-14 — RBAC T-02 remains explicitly OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION Teacher→unassigned student CURRENT OBSERVED BEHAVIOR DENIED PRODUCT DECISION OPEN must be resolved before RBAC implementation reaches this scope — empirical verification deferred to F2 — do NOT change. **Blockers:** high-cost open decisions O-01,O-02,O-08,O-09,O-10,O-13,O-14 must be resolved before Laravel schema/API; Telegram/Bale/Backup/Mobile integration contracts B require backend/integration layer but are REQUIRED PRODUCT CAPABILITY not optional; Finance/Reports I2/D6 remains C deferred; per_page 200 ceiling I16 mitigation disclosure not removal; I13 view-boundary not closure remaining 3 readers; I15 error discarding 14 consumers; external browser QA L4 NOT VERIFIED. **Exact next slice:** **F1 — Learning + Level Access + Library + Gallery** — exactly ONE NEXT ACTION — canonical rule Level N=>1..N owner learning/eligibility.ts, scope OPEN global vs per-program vs per-instrument, library filtering type/level/visibility/search/filter/sort/preview/locked/metadata workflows + detail view same surface per F1 disposition + publication status active/visibility deferred workflow per F1 disposition + inactive-archived OPEN per F1 disposition, gallery genuine vs shallow audit + upload via media seam + seed VERIFIED 2 albums 0 images per F1 disposition, absorb minor hardening Classes deep-link get(id) authoritative, pagination per_page mitigation, media one-frame issue into F1/F2. **Backend intentionally deferred:** Laravel/backend/migrations/controllers/routes/models, Finance/Reports domain implementation money precision idempotency gateway, notification server wiring, localization/settings server wiring, working-hours/session-rules server wiring, free-slot search, Telegram/Bale/Mobile integration implementation (contracts now, implementation later — REQUIRED), media storage provider S3/MinIO signed URLs scanning, export server streaming, dashboard server aggregation, RBAC backend enforcement org_id + permission + scope + per-object auth, student portal auth identity linking per-user read cursor, backup encryption retention integrity restore. Frontend remains docs-only until F1 implementation authorized. **Canonical engineering docs updated:** PROJECT_STATE.md, PHASES.md, OPEN_ITEMS.md, SESSION_HANDOFF.md point same CURRENT_BRANCH arena/frontend-completion-spec CURRENT_HEAD a251e6942b2a645987286de6e822607554e84df9 PLANNING_CHECKPOINT e57bf1922e4ca3b80796715028445cfe8a077c9c PREVIOUS_PLANNING_CHECKPOINT c244fec142fcc35faff8e8548c8ebbc05cb53bf5 BEFORE a251e6942b2a645987286de6e822607554e84df9 AFTER new correction commit spec `docs/frontend-completion/` v14 @ `e57bf19` planning checkpoint + corrections + F1-KICKOFF PR #4 F0 complete F1 next — SHA semantics no longer ambiguous — BEFORE a251e69 AFTER new commit PLANNING e57bf19 PREVIOUS c244fec. **SESSION_HANDOFF machine-readable CONTINUE HERE section required — see docs/engineering/SESSION_HANDOFF.md — CURRENT_BRANCH arena/frontend-completion-spec CURRENT_HEAD a251e6942b2a645987286de6e822607554e84df9 PLANNING_CHECKPOINT e57bf19 PREVIOUS_PLANNING_CHECKPOINT c244fec BEFORE a251e69 AFTER new commit NEXT_ACTION F1 — SHA semantics no longer ambiguous — DoD 15 items — F1 dispositions 4 items — gallery seed 2 albums 0 images VERIFIED — C-2 REQUIRED — C-4 BEFORE a251e69 AFTER new commit — T-02 OPEN EVIDENCE CONFLICTING.**

> **F1-KICKOFF CORRECTION — 2026-09-19 — CURRENT_BRANCH arena/frontend-completion-spec CURRENT_HEAD a251e6942b2a645987286de6e822607554e84df9 PLANNING_CHECKPOINT e57bf1922e4ca3b80796715028445cfe8a077c9c PREVIOUS_PLANNING_CHECKPOINT c244fec142fcc35faff8e8548c8ebbc05cb53bf5 BEFORE a251e6942b2a645987286de6e822607554e84df9 AFTER new correction commit — previous correction commits 9695097 + a251e69 documentation-only NOT planning milestones — PLANNING_CHECKPOINT = e57bf19 remains last completed planning checkpoint — CURRENT_HEAD = a251e69 actual Git HEAD before this F1-KICKOFF correction — SHA semantics no longer ambiguous: BEFORE a251e69 AFTER new commit PLANNING e57bf19 PREVIOUS c244fec — do NOT call e57bf19 the current Git HEAD — reusable DoD 15 items — F1 dispositions Library detail view same surface / publication status active/visibility deferred workflow / inactive-archived OPEN / gallery seed 2 albums 0 images VERIFIED — C-2 Telegram Backup REQUIRED preserved — C-4 BEFORE a251e69 AFTER new commit PLANNING e57bf19 PREVIOUS c244fec — T-02 OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION — NEXT ACTION exactly ONE F1 — no src/tests/package/backend changes — no branch/history rewrite — push only arena/frontend-completion-spec — do NOT merge PR #4 — do NOT change product decisions/architecture/roadmap/F1 definition/RBAC T-02/accepted/open/historical.**



> **Last state update: 2026-09-25 — D9 A-lite and the re-baselined performance budget (this pass's own record).** The D9 track on `arena/01a0cef8-parsian-music-dashboard-opus` closed: the `tailwind-merge` dependency was replaced by the in-repo `mergeClassNames` rule table (owner-accepted; `cn()`'s public signature and every call site unchanged), the store-only ZIP attempt was reverted byte-for-byte, and the **total** gzip baseline asserted by `src/__tests__/bundleBudget.test.ts` was re-baselined on the post-reduction measurement — baseline **343 947 B**, cap **361 144 B** — recorded in [DECISIONS.md](DECISIONS.md) §22. Measured total at the checkpoint: **343 947 B**; entry chunk **126 979 B** (still strictly below its `c16890f` anchor 273 791 B); full suite at the recorded environmental failure set with **no new or unknown failure**. The work was finalized as `046771d` and pushed fast-forward (`bcacdcb..046771d`); `main` (`ece060d`) is untouched and **Laravel/backend remains ❌ NOT STARTED and NOT AUTHORIZED**.

> **Last state update: 2026-09-21** — the **D2 pass**, which records the consolidation pass and the **D1** documentation checkpoint (`6ca25be654b56579054c88cd498a42f57ea63582`, committed locally, push pending) on top of it (see the blockquotes above): canonical branch is `main`, consolidation base `f63ebaab37096fcb25063aca51c713e26d3b3da7`, F0–F10 and the governance checkpoint are on `main`, PR #5 merged and PR #4 closed unmerged. **Everything below this line is the older freeze-line record and is kept for audit — the blockquote that follows describes the state at 2026-09-17:** **M10 (fixture / type / seed separation, D5) is ✅ COMPLETE** *(M11's record follows below it)*:
> the two design-time fixture modules — records.ts and academy.ts, dissolved at this checkpoint — are
> closed by **one** implementation checkpoint, `e7a64b7777be36ddd97fc3337898fad794118e5b`
> (*fix(reliability): M10 — dissolve the fixture boundary, no architectural sociology* — 82 files,
> 2 122 insertions / 1 942 deletions, including one new 13-check boundary gate and twelve new owner
> modules), **explicitly authorized by the owner** after the D5 finalization and built on
> `98af31b5b04022362f0bc95ca99c426f2d7c8446` (the D5 finalization record, the twenty-ninth
> documentation checkpoint by this ledger's count, and therefore M10's **effective safe rollback boundary**). The four D5
> categories landed with exactly one owner each — types with their domains, the canonical DEMO seed at
> `src/domains/demo/academySeed.ts` (data unchanged), fabricated UI measurements **removed** and
> classified one by one in the Finance/Reports deferral ledger (`src/lib/financeReportsDeferral.ts`,
> **D6**/**I2**) with tombstones enforced by `src/__tests__/m10Boundary.test.ts`, and presentation
> vocabulary/config in named single owners (§3 → "Phase status" lists them). `freeSlotsTuesday` is gone
> from executable production data while its palette question survives as an **honest typed no-data**
> result; the Hero's identity comes from the M8 branding seam and `useAuth` — **no `BrandingSettings`
> persistence or schema change** — and its fabricated status/peak claims are removed in favour of
> scheduling-seam reads or silence. **What M10 did not do** is just as recorded: no Finance or Reports
> domain (**I2 remains deferred**), no free-slot search backend, no backend work, no branding
> persistence redesign. **M11 remains ❌ NOT STARTED and not authorized**, with it the milestone order
> (M8 → M9 → **M10** → M11). *(Superseded, 2026-09-17: M11 was authorized afterwards and is ✅
> **COMPLETE** across four checkpoints ending at `f7eb867` on the boundary `3ed0bfe` — its record is the
> M11 closure paragraph at the end of this blockquote, §3 → "Phase status", §4 → "M11 validation" and
> §7 item 23; the phase specification names no milestone beyond M11, so the next phase is NOT STARTED
> and NOT AUTHORIZED. The sentence above is kept as the state at M10's closure, which is what it
> describes.)* See §3 → "Phase status", §4 → "M10 validation" and §7 item 22;
> **browser QA NOT VERIFIED** (§5). **M9 stays ✅ COMPLETE** — its registration stands as written in
> [PHASES.md](PHASES.md) and in §3, and this file's older M9 closure text below is superseded as *last
> state* only, not as history.
>
...
>
> **M11 decision pass (2026-09-16, documents only): the three decisions gating M11 — D7, D8, D9 — are
> RECORDED** in [DECISIONS.md](DECISIONS.md) §19 by the owner's decision-record pass: D7's
> dependency-free accessibility strategy, D8's api-hybrid disclosure behaviour (eleven
> demo-in-both-modes getters, the registry itself cited D8's name first), and D9's
> **measurement-derived** budget shape on a real build baseline taken at `c16890f` (single JS chunk
> raw 984 476 B / gzip 273 791 B; CSS raw 115 373 B / gzip 16 453 B; `index.html` 1 885 B / gzip
> 999 B; total `dist/` 2 188 993 B; 14 static view imports, zero `lazy`/`Suspense`; weights 300 and
> 800 provably unused). The browser-QA environment disposition is recorded in §5: **no browser tooling
> exists or may be installed in this environment**, so L4's checklist stands as explicitly
> **NOT VERIFIED** unless an external QA environment is separately authorized. **M11 remains ❌ NOT
> STARTED and NOT AUTHORIZED** — a recording pass is not an implementation authorization; all ten
> order/protected-area constraints of the spec stand as written. This commit is the **thirty-first**
> documentation checkpoint *by this ledger's count* and names `c16890f` as the **thirtieth**.
>
> **M11 closure (2026-09-17, documents only — the thirty-second documentation checkpoint by this
> ledger's count, which cannot name itself).** **M11 (performance I6, api-hybrid disclosure D8,
> accessibility D7, browser-QA checklist L4) is ✅ COMPLETE.** The owner authorized the exact scope
> on 2026-09-17, and it landed across **four** product commits on top of the decision record — the
> milestone's **starting SHA is `3ed0bfe4ac928451a4bddf9de3a34f7be681c33f` and its final
> implementation HEAD is `f7eb86762dc11f7d5dd3c7218e7c697fad29488e`** (an implementation report
> wording that named `f7eb867` the "starting SHA" is **corrected** here: `f7eb867` is the final
> HEAD, never the start; the implementation history itself is untouched): `3eed4f19` **perf** (route
> code-splitting in lazy academic/operations workspace chunks, React-dependent **vendor** chunk,
> `chunkSizeWarningLimit` deliberately untouched, the provably unused Vazirmatn **300/800** imports
> removed), `24998d83` **test** (the D9 budget gate encoded from the M11 measurements themselves),
> `53794d57` **feat** (**D8** — the single `DEMO_SERVED_DOMAINS` enumeration at the registry's
> composition root and the persistent, non-dismissable, api-mode-only, health-claim-free disclosure
> in the shell and on login, pinned by six tests including the stale-enumeration tripwire) and
> `f7eb86762` **test** (**D7** — the dependency-free accessibility gate: accessible-identity by
> role/name, focus-trap entry/loop/escape/return, command-palette keyboard flow,
> `prefers-reduced-motion` and RTL/Persian invariants; 14 cases, stable across three runs). **The
> final D9 values are measured and recorded** (CLI `gzip -6 -n` per file, the c16890f baseline
> method, at `f7eb867`): entry JS gzip **108 207 B** (baseline 273 791 B, −60.5%), vendor gzip
> **60 108 B**, total JS+CSS+HTML gzip **298 249 B** (within the +5% allowance: cap 305 805 B),
> largest single chunk gzip **108 207 B** (no chunk ≥ 400 000 B), `dist/` raw **1 964 789 B** — and
> the >500 kB raw chunk warning no longer fires, cleared by real splitting, **not** by raising the
> threshold. **L4 is recorded exactly as NOT VERIFIED**, with the external-QA disposition preserved
> (§5): no browser tooling exists or was installed, so no browser checklist ran and none is
> claimed. **Protected areas verified in §4:** Finance/Reports (`I2`), `demo-data.md` (**L7**),
> backend/Compensation/Attendance/persistence plane/CSP/M10 boundary gates untouched; package
> manifests absent from the implementation diff; **M10 (`e7a64b7`) intact in history**. Test status
> as measured at `f7eb867`: 144 files, **2 020 passed / 7 failed** — the seven failures are the
> **known environmental `projectState` checkpoint-object class** (this shallow clone lacks the
> recorded historical SHA objects; the test is **not** touched, per §7 item 22's inheritance) —
> with `npm run typecheck`, `npm run build` and `git diff --check` clean. **Accepted with recorded
> limitations** (§7 item 23 — including the lazy-loading **first-paint residual**); **M12 is ❌ NOT
> STARTED and NOT AUTHORIZED** — the phase specification names no milestone beyond M11, and nothing
> in this registration pass authorizes any further work. This commit also names `3ed0bfe` as the
> **thirty-first** documentation checkpoint — the commit **M11's implementation chain was built on**
> and therefore M11's **effective safe rollback boundary**.
...
| Previous documentation checkpoint | `3ed0bfe4ac928451a4bddf9de3a34f7be681c33f` — **the D7/D8/D9 decision record** (*docs(m11): record D7/D8/D9 decisions and the browser-QA environment disposition*): the three decisions gating M11 were recorded ahead of any implementation authorization — D7's dependency-free accessibility strategy, D8's api-hybrid disclosure behaviour on the **eleven** demo-in-both-modes getters, and D9's measurement-derived budget shape on the real `c16890f` baseline — and the browser-QA environment disposition (**NOT VERIFIED** is this environment's only honest disposition) was recorded in §5. The record explicitly **did not authorize** M11. **The thirty-first** documentation checkpoint *by this ledger's count*, which could not name itself — the M11 closure pass names it — and the commit **M11's implementation chain was built on**, which makes it M11's **effective safe rollback boundary** |
| Documentation checkpoint before that | `c16890f5814d6ff40cd95f59be2214d2f1bcf82e` — **M10's documentation closure** (*docs(m10): close the fixture-dissolution phase and register the M10 checkpoint*): M10 registered ✅ **COMPLETE** against its measured evidence — the implementation checkpoint `e7a64b7777be36ddd97fc3337898fad794118e5b`, the §4 validation block and the two errata kept recorded rather than smoothed over. Documents only — eight authorized files; no product source, no test, no dependency, no gate. **The thirtieth** documentation checkpoint *by this ledger's count*, which could not name itself — the D7/D8/D9 decision-record pass names it — and the commit **the D7/D8/D9 record was built on** |
| Documentation checkpoint before that | `98af31b5b04022362f0bc95ca99c426f2d7c8446` — **the D5 finalization record** (*docs(governance): finalize decision D5 — four-category model and seventeen owner clauses*): the separation shape M10 executed was recorded and finalized, the earlier governance passes' stale claims were corrected (L3 scope, the "third role already unused" premise, the fixture line/import census), and **L2** plus the five-name drift table were left scoped for M10. Documents only; no product source, no test, no dependency, no gate. **The twenty-ninth** documentation checkpoint *by this ledger's count*, which could not name itself — the M10 closure pass named it — and **the commit M10 was built on** |
| Documentation checkpoint before that | `c186baaa1dda39e274369ed0387094822b00f0ae` — **M8's documentation closure** (*docs(m8): reconcile the branding phase record and register the milestone*): M8 registered ✅ **COMPLETE** against its measured evidence, **D2** reconciled now that the milestone had landed, and the milestone's own limitations and out-of-scope residue kept recorded rather than smoothed over. Documents only — four `docs/engineering/` documents; no product source, no test, no dependency, no gate. **The twenty-seventh** documentation checkpoint, which could not name itself — the M9 closure pass names it — and the commit **M9 was built on** |
...
- A **documentation checkpoint** is a pushed commit that changes documents and validation gates
  but no product behaviour. **Thirty-one** are **named** so far — the thirty-second is the commit carrying
...
future SHA or an invented one fails the suite instead of misleading the next reader. One recorded
exception exists: the two preserved, unmerged branch tips registered by full SHA in §2
(`251af96f…` and `94d32de3…`) are real commits that history deliberately never merged, so
reachability can never hold for them without breaking the preservation rule. The gate carries them
as a closed, self-guarding exception — each must still be registered above by full SHA and still
exist as a commit, so no other SHA is covered and the exception cannot outlive its evidence
...
| Field | Value |
|---|---|
| Current phase | **Post-consolidation documentation gate (D1/D2)** — canonical working branch `main`, consolidation base `f63ebaab37096fcb25063aca51c713e26d3b3da7` (PR #5 merge), **D1 documentation checkpoint `6ca25be654b56579054c88cd498a42f57ea63582` on top of it and D2 being recorded now**. *Preceding completed phase (preserved):* **Arena Frontend Freeze — M-1..M-6, D7, D9 CLOSED at `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb` on `arena/01a0b6be-parsian-music-dashboard-opus` (2026-09-19)** |
| Phase status | ✅ **COMPLETE — Arena Frontend Freeze at `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb` (2026-09-19, `arena/01a0b6be-parsian-music-dashboard-opus`) — documentation-only closure.** **M-1 Quick Actions CLOSED**, **D7 Accessibility CLOSED**, **M-2 Teacher Photo CLOSED**, **M-3 Student Photo CLOSED**, **M-4 I15 Failure Disclosure CLOSED**, **M-5 I16 Deep Links CLOSED**, **M-6 Settings Honesty CLOSED**, **D9 Performance CLOSED**. Frozen HEAD `a3867a6` includes: `src/components/ds/patterns.tsx` Toggle disabled, `src/views/Settings.tsx` honest deferral Surfaces (notifications 2, localization 1, session rules 1, working hours expanded) with 7 notification toggles disabled, 6 localization selects disabled, 4 session-rules inputs disabled, Friday toggle disabled "— غیرفعال". Typecheck `npx tsc --noEmit --skipLibCheck` 0 errors, build 2037 modules entry gzip 16.39kB CSS, vendor 60.29kB, index 118.50kB, total ~300kB within +5% cap, no chunk ≥400kB. Focused 92 pass, full suite 2102 passed / 10 failed — 10 failures are **known historical** `projectState.test.ts` SHA/branch expectations, test not altered. **Browser QA NOT VERIFIED** as formal final-audit disposition. **Deferred does NOT modify frozen frontend:** Classes deep-link beyond capped list, Finance/Reports backend/domain, notification server wiring, localization/settings server wiring, working-hours/session-rules server wiring, free-slot search, external browser QA. **Backend boundary:** frontend freeze precedes Laravel/backend implementation. **Canonical branch policy:** all future frontend fixes require explicit decision to reopen freeze. Previous: M11 (performance I6, D8, D7, L4) at `f7eb867` on `3ed0bfe`, M10 at `e7a64b7` on `98af31b` — preserved. |
| Previous phase status (the row this pass advanced past) | ✅ **COMPLETE** — **M10** (fixture / type / seed separation, **D5**) was implemented in **one** checkpoint, `e7a64b7777be36ddd97fc3337898fad794118e5b` (*fix(reliability): M10 — dissolve the fixture boundary, no architectural sociology* — 82 files, 2 122 insertions / 1 942 deletions: the two design-time fixture modules dissolved, twelve new owner modules, one new 13-check boundary gate, sixteen fixture-era gates mechanically re-derived), **explicitly authorized by the owner** after the D5 finalization and built on `98af31b5b04022362f0bc95ca99c426f2d7c8446` — the D5 finalization record, the twenty-ninth documentation checkpoint by this ledger's count, which is therefore M10's **effective safe rollback boundary** — and this documentation pass reconciles the record. **The four D5 categories found exactly one owner each.** **A — domain/entity types** live with their domains: `Student`/`StudentNote` in `src/domains/students/types.ts`, `Teacher`/`TeacherNote` in `src/domains/teachers/types.ts`, `AcademyClass` in `src/domains/classes/types.ts`, `Resource` in `src/domains/library/types.ts`, `Conversation` in `src/domains/demo/types.ts`, `InstrumentId` in `src/domains/instruments/types.ts`, and the timeline's `ClassSession`/`ClassSessionStatus` in `src/domains/scheduling/types.ts` (the session lifecycle `Session` stayed put — a relocation and a D5 disambiguation rename, **no field removed**, see §7 item 22's errata). **B — the canonical DEMO seed** lives at `src/domains/demo/academySeed.ts` (data unchanged), consumed only by the demo layer. **C — fabricated UI data/measurements** are **removed**: every one is classified item-by-item in the Finance/Reports deferral ledger (`src/lib/financeReportsDeferral.ts`, **D6**/**I2**) and tombstoned against quiet return by `src/__tests__/m10Boundary.test.ts`. **D — static vocabulary and presentation configuration** have named single owners: `src/lib/navigation.ts`, `src/lib/viewContracts.ts`, `src/lib/financeVocabulary.ts`, `src/domains/scheduling/weekdays.ts`, `src/views/messages/composerTemplates.ts`, `src/components/overlays/commands.ts`, `src/components/ds/samples.ts` (design-system-plated config). **The free-slot question survived its answer:** `freeSlotsTuesday` is gone from executable production data and `nlCommands` n3 (the Tuesday free-class question) now returns the product's honest typed no-data result, opening the schedule — **no free-slot search system was built** and no fabricated availability figure remains. **Hero is honest without a new identity store:** the academy name/tagline come from the M8 `useBranding` seam (**D2**), the greeting from `useAuth` — extending **no** `BrandingSettings` persistence or schema — and the hardcoded healthy-status badge, the fabricated peak activity and the fixed attention point are removed, replaced by scheduling-seam reads (`useDayPulse`) or silence. **F2/I8 preserved:** zero diff anywhere under the compensation and attendance domains or the persistence/backup plane; the protected suites pass. **What M10 did NOT do:** no Finance or Reports domain — **I2 remains deferred** — and the two surfaces carry their deferral explicitly; no free-slot backend; no branding persistence redesign; no backend work; no run of browser QA (§5). **Accepted with recorded limitations** — see §4 → "M10 validation" and §7 item 22; **browser QA NOT VERIFIED** (§5). |
| Earlier phase status (kept as written from the row two passes above advanced past) | ✅ **COMPLETE** — **M9** (dashboard insight from live data) was implemented in **one** checkpoint, `8d34eb3d1cd639cffc794596250c897b5ed4b6b3` (*feat(m9): derive every dashboard insight from stored records, guarding the empty series first* — 14 files, 3 025/328: eight modified files and six new ones, four of them test files carrying **61 cases**), **explicitly authorized by the owner on 2026-09-16** and built on `c186baaa1dda39e274369ed0387094822b00f0ae` — M8's documentation closure, which is therefore M9's **effective safe rollback boundary** — and this documentation pass reconciles the record. **The two identifiers it closes were closed in the order the milestone's own spec required: I9 first**, so the empty-series crash could not be reached by the live data H4 was about to feed in, **then H4**. Every figure the four insight panels and the dashboard show is derived from the records of the environment being read, through existing seams (`useAcademyMetrics` for the hero metrics, the new read set in `src/domains/shared/useDashboardInsights.ts` for the panels, and `src/lib/stats.ts`'s `meanOf`/`ratioPct`/`topBy` for the aggregates); the fixture sources (`signals`, `growthSeries`, `revenueSeries`, `occupancy`, `quickActions`, `todayFlowIds`, `attentionItems`, `attentionQueue`, `intelligenceCards`) no longer reach those surfaces, and an input set with no records yields the product's typed no-value glyph rather than a fabricated figure or sentence. **Collected revenue was the one measure with no authoritative seam** — Finance and Reports are README-only and `invoices`/`payments` have no repository — so the chart was **removed** and the money slot shows the receivables the student records carry; **no Finance domain, no repository and no fabricated revenue metric were created.** **Accepted with recorded limitations** — see §3 → "Last completed work (M9…)", §4 → "M9 validation" and §7 item 21; **browser QA NOT VERIFIED** (§5). |
| Earlier phase status (kept as written from the row the pass above advanced past) | ✅ **COMPLETE** — **M8** (branding application & visual identity, **D2**) was implemented in **one** checkpoint, `735617d0324a8f4ba2e243846eedf069d389751a` (*feat(m8): render the persisted branding in the shell, the login screen and the tokens* — 6 files, 360/26, one new test file with **15 cases**), **explicitly authorized by the owner on 2026-09-16** and built on `a039ab0e2d1f0d339b91b23c6dffc3b0a4cc5046` — the pre-M8 boundary, which is therefore M8's **effective safe rollback boundary** — and its record was reconciled at `c186baa`. The persisted `BrandingSettings` is now the identity the product renders: `applyBranding`/`useApplyBranding` (`src/domains/branding/useBranding.ts`) is mounted in `src/App.tsx` below the lifecycle gate and above `AuthProvider`, the four `--brand-*` custom properties are consumed by the design-system tokens in `src/index.css`, and `src/components/layout/Sidebar.tsx` and `src/views/Login.tsx` read `branding.academyName` / `branding.tagline` instead of the demo fixture. All pushed. **Accepted with recorded limitations** — see §3 → "Last completed work (M8…)", §4 → "M8 validation" and §7 item 20; **browser QA NOT VERIFIED** (§5). |
...
| Next phase | No beyond-M11 milestone is specified in the product phase sequence, so the ledger position is **M12 — ❌ NOT STARTED, NOT AUTHORIZED**: no specification, no decision gate, and no separate owner authorization exists, and this M11 registration pass grants none. **L2 stays open** alongside **L7**, **I2**, **I19** and the other recorded residuals — nothing here closes them |
...
`a039ab0`) and **M9** (one implementation checkpoint `8d34eb3` — dashboard insight from live data,
authorized by the owner on 2026-09-16 and built on M8's documentation closure `c186baa`, with the **I9**
guards landing first) are
...
§4 → "M8 validation") — after which **M9 landed on top of that closure** — one implementation checkpoint, built on
`c186baa`, with **I9 first** and **H4** after it — so **M10 and M11 have not started**: the ledger in
[PHASES.md](PHASES.md) marks the product-feature phase's remaining milestones NOT STARTED, and **M10 is
not authorized**.
...
src/data/records.ts (the design-time fixture module, dissolved at M10) — a frozen weekday as "today", a hardcoded room-4 transfer narrative and
...
src/data/records.ts (the design-time fixture module, dissolved at M10): eight registers keyed by legacy ids `g7`–`g15` that match no session the
...
- **CP4 `f1ec0dd`** — class relations and the navigation counts: instructor and studio resolve through
  ...
  the chrome: `NavDef.badge` is **removed** from src/data/academy.ts (the design-time fixture module, dissolved at M10), the messages badge is the sum of
...
deletions — the three views, src/data/academy.ts (the design-time fixture module, dissolved at M10), `src/components/layout/Sidebar.tsx`, two new modules
...
(**M9**/**M10** — *(correction, 2026-09-16: the panels' half was closed by **M9** at
`8d34eb3d1cd639cffc794596250c897b5ed4b6b3`; `Library`, `Settings` and the chat templates remain, with
**M10**)*), and **browser QA has never run** (§5) — the navigation badge, the three rewired
...
integration edits (`ViewId`/nav label in src/data/academy.ts (the design-time fixture module, dissolved at M10), the `compensation` entry in
...
src/data/academy.ts (the design-time fixture module, dissolved at M10)) is recorded at §7 item 20; **M9 landed without touching it** — the hero and the
seed were outside its scope, which §7 item 21 records — so it stays with **M10**.

### Last completed work (M9, in one paragraph)

M9 — **dashboard insight from live data** — was authorized by the owner on 2026-09-16 and landed in **one**
implementation checkpoint, `8d34eb3d1cd639cffc794596250c897b5ed4b6b3`, built on **M8's documentation
closure** `c186baaa1dda39e274369ed0387094822b00f0ae` (the twenty-seventh documentation checkpoint, and
therefore M9's effective safe rollback boundary). It closed two recorded items in the order its own spec
required. **I9 first:** `Sparkline` (`src/components/ds/primitives.tsx`) used to compute
`Math.min(...data)` / `Math.max(...data)`, i.e. ±Infinity and degenerate coordinates on `[]`, and
`BusinessIntelligence` indexed `revenueSeries[n - 1]`; because every caller passed a static fixture the
crash was unreachable — until H4 fed those components real, possibly empty, series. Both now take the
absence explicitly (`readonly number[] | null` on the primitive, a per-chart empty-set guard before any
index or `Math.min`), render the product's one no-value glyph `NO_DATA` (`src/lib/format.ts:35`) in place
of a chart, and are covered by focused `[]` cases that assert the *rendered* result, not merely the absence
of a throw. **Then H4:** the four insight panels and the dashboard read no fixture value any more. A new
pure derivation module (`src/domains/shared/dashboardInsights.ts`) and one read set
(`src/domains/shared/useDashboardInsights.ts`, five list reads each stating `per_page: 500`) feed them, and
`src/views/Dashboard.tsx` reads the academy clock once (`useAcademyNow()` + `academyIsoDate()`) and passes
the derived models down; `signals`, `growthSeries`, `revenueSeries`, `occupancy`, `quickActions`,
`todayFlowIds`, `attentionItems`, `attentionQueue` and `intelligenceCards` no longer reach those surfaces.
Every displayed figure is traced to a record: the at-risk count and the mean attendance come from the
student rows themselves (the mean over rows with `sessionsTotal > 0`, deliberately *not* the
`attendanceRatePct` hero metric, which P1 caps), sessions and cancellations from the bounded `useSessions`
window, overdue balances and waitlists from students and classes, today's rows, their labels and the
seeded room clash from the sessions/classes/rooms/teachers reads, receivables from `Student.balance`,
roster growth from `Student.since`, occupancy from class seats and the stored weekdays, and the instrument
mix from `Student.instrument` — with `meanOf`, `ratioPct` and `topBy` (`src/lib/stats.ts`) doing the
aggregation so an empty input set yields `null` rather than a number. **The revenue measure had no
authoritative seam** — Finance and Reports are README-only and `invoices`/`payments` have no repository —
so rather than inventing a domain, a repository or a figure, the chart was **removed** and the money slot
now shows the receivables the records actually carry; the milestone did **not** implement Finance or
Reports. EMPTY stays first-class: every panel renders its structure and states «داده‌ای نیست», the two
measures with no history render `NO_DATA` glyphs, and no `NaN`, `Infinity`, retired fixture figure or
retired fixture sentence reaches the DOM — asserted over the real `Dashboard` in both DEMO and EMPTY.
Fourteen files (eight modified, six new — four of them tests carrying **61 cases**); **no dependency, no
route change and no submission change**; **browser QA is NOT VERIFIED** (§5), and the limitations this
milestone did *not* close are §7 item 21.
...
### M11 validation (measured at `f7eb86762dc11f7d5dd3c7218e7c697fad29488e`, the milestone's final implementation checkpoint)

M11 landed across **four** product commits on top of the D7/D8/D9 decision record `3ed0bfe4ac928451a4bddf9de3a34f7be681c33f` — which is therefore M11's **effective safe rollback boundary** (rolling back to it drops M11 entirely and keeps the decision record, M10 and everything before). **Starting SHA = `3ed0bfe`; final implementation HEAD = `f7eb867`** — an implementation-report prose line that labelled `f7eb867` the "starting SHA" is **corrected** by this row; the implementation commits themselves were never amended. Every number below was measured on the `f7eb867` tree:

| Check | Result |
|---|---|
| Starting point | ✅ branch `arena/01a0aa83-parsian-music-dashboard-opus` at `3ed0bfe4ac928451a4bddf9de3a34f7be681c33f`, clean, the M10 chain — `e7a64b7777be36ddd97fc3337898fad794118e5b` intact — behind it; sandbox re-clone mid-pass disclosed non-destructively (branch pointer re-attached at the pushed `f7eb867` and the tree verified byte-identical to the push state; **no** commit, ref or history changed by that recovery) |
| Phase A — baseline | ✅ `npm run build` at `3ed0bfe` measured and confirmed the recorded D9 baseline before any source change: single JS chunk raw 984 476 B / **gzip 273 791 B**, deterministic across rebuilds (method: CLI `gzip -6 -c`) |
| Phase B — I6 perf | ✅ view-level lazy mounting in two workspace groups (`academicViews` 49.8 kB gz, `operationsViews` 46.2 kB gz at landing), React/scheduler **vendor** chunk 60.1 kB gz; first-paint Login/Dashboard stay eager; `Suspense` inside `ViewOutlet` keeps the shell mounted; `chunkSizeWarningLimit` **not** raised; protected route protection / EMPTY / CSP behaviour unchanged |
| Phase C — D9 gate | ✅ `src/__tests__/bundleBudget.test.ts` **8/8** — encoded from the M11 measurements, not backfilled from the baseline: entry strictly below 273 791 B, total within +5% (cap 305 805 B), no chunk ≥ 400 000 B, weights 300/800 absent from `dist/`, split still real (academic/operations/vendor chunks exist), `chunkSizeWarningLimit` assignment and sourcemap re-enabling are themselves failures |
| Phase D — D8 | ✅ `src/components/shell/DemoBackedNotice.tsx` + registry's single `DEMO_SERVED_DOMAINS` enumeration (the **eleven** domains, no second list); **6/6** — renders in api mode, absent in demo, non-dismissable, wording equals the registry set, no fabricated health/ping/connected claims, tripwire fails if a listed getter stops resolving demo |
| Phase E — D7 gate | ✅ `src/__tests__/a11yGate.test.tsx` **14/14**, dependency-free — accessible identity on Dialog/Drawer/palette/sidebar/login (role + Persian name), focus-trap entry/loop/Escape/return-to-opener, command-palette keyboard flow (focus entry, arrows, Enter-runs, Escape-dismisses), `prefers-reduced-motion: reduce` and `[data-motion="off"]` zero-duration contracts, `lang="fa" dir="rtl"`. Stable across **3 consecutive runs**. No axe/new dependency |
| Phase F — L4 | ❌ **NOT VERIFIED** — tooling census re-run at closure (`package.json`/lockfile grep = 0 for Playwright/Puppeteer/Cypress/WebDriver; no browser binaries; `dist/` present but never rendered in a browser): the checklist cannot run here, no tooling was installed, **no browser verification is claimed**. The external-QA disposition stands (§5) |
| Final D9 measurements | ✅ entry JS gzip **108 207** B; vendor gzip **60 108** B; total JS+CSS+HTML gzip **298 249** B; largest chunk gzip **108 207** B; `dist/` raw **1 964 789** B (baseline 2 188 993 B, −10.2%); Vite's >500 kB raw warning silent |
| Protected-area diff | ✅ `git diff e7a64b7..f7eb867 --name-only` shows **no** `src/views/Finance.tsx`, `src/views/Reports.tsx` (I2), `docs/architecture/demo-data.md` (L7), backend/server file, persistence/backup-plane file, or CSP string under `deploy/` or in `PRODUCTION_CSP`; Compensation and Attendance semantics, scheduling semantics, M10 boundary guarantees and fixture-era protections untouched; `package.json`/`package-lock.json` absent from the diff (no dependency change) |
| Focused / protected suites | ✅ 89 (routeProtection + configGate + cspCompatibility + architectureBoundaries + m10Boundary 13/13 + emptyEnvironment + brandingApplication) at Phase B; 62 + 24 at Phase D; `writeFeedbackHonesty` green; `m10Boundary` **13/13** reaffirmed at closure |
| Full suite at `f7eb867` | **144 files — 2 020 passed / 7 failed of 2 027**, duration ≈ 178 s. The seven failures are exactly the **known environmental `projectState` checkpoint-object class**: this clone is shallow-squashed (verified `git rev-parse --is-shallow-repository` → `true`; `33b1031…`, `aca40c5…`, `292b8b8…` absent as objects), so the seven checkpoint-existence/ancestry checks cannot resolve their recorded SHAs. The set is **identical** to the pre-implementation baseline — proven by stash-compare during the pass — the test is **not** modified and nothing is claimed as a full-suite green |
| `npm run typecheck` | ✅ clean (zero errors) at every phase boundary |
| `npm run build` | ✅ exit 0 (~4 s); zero chunk-size warnings after the split |
| `git diff --check` | ✅ clean |
| Remote | ✅ pushed fast-forward after each phase (`3ed0bfe..3eed4f1..24998d8..53794d5..f7eb867`), no amend, no force-push, no reset |

### Arena Frontend Freeze validation (frozen at `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb`, canonical `arena/01a0b6be-parsian-music-dashboard-opus`, 2026-09-19 — documentation-only closure)

Frontend phase formally closed at this SHA. **M-1 Quick Actions CLOSED**, **D7 Accessibility CLOSED**, **M-2 Teacher Photo CLOSED**, **M-3 Student Photo CLOSED**, **M-4 I15 Failure Disclosure CLOSED**, **M-5 I16 Deep Links CLOSED**, **M-6 Settings Honesty CLOSED**, **D9 Performance CLOSED**. No production code or tests changed in this pass — documentation only.

| Check | Result |
|---|---|
| Frozen HEAD | ✅ `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb` on `arena/01a0b6be-parsian-music-dashboard-opus`, worktree clean, `git ls-remote --heads origin` confirms canonical SHA identical, other branches unchanged (main `a6469759fea743b9332702bb9a20e69fc4abc1be` — *as verified at the 2026-09-19 freeze; **superseded 2026-09-20: `main` has since advanced to `f63ebaab37096fcb25063aca51c713e26d3b3da7` by the PR #5 merge**, and `a646975` remains its previous tip and an ancestor*, `arena/01a0aa83` `d31cbe9d83ce327d4c1b14e352f2de1b0a6c16b8`, `arena/01a0b059` `251af96f405df5935ee17ddc9ba6b490f7989c2d`, `handoff/arena-frontend-pre-backend` `94d32de3220cee314606c0c4705e20e0fdcfb2af`) |
| Ancestry | ✅ GitHub `compare` API confirms chain includes `d31cbe9d83ce327d4c1b14e352f2de1b0a6c16b8` (arena/01a0aa83 tip), `d6430df272cf6b6b88efd9d68e0387fc353e39c3`, `20d8f6e2c0984a924cbe526ba55004fd5d3b4523`, `6600f5c100a152c5dba9d29d460d30ee46b062da`, `bde7e5fa44cbb2c9b8547f6545843638505e104d`, `2a4ef2e04eccaacfd56995910362d646a1bed02a`, `eaccbb1f45ffdc577ddaf838dde81fe2ed33a13f`, `3ddec8e2b368f133a2a914a4a46d3844c77212b9`, `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb` — all ahead/identical, no history rewrite |
| M-1 | ✅ `quickActions` no hardcoded option arrays, student→`StudentFormDialog`, class→`ClassFormDialog`, message→Messages, payment→honest Finance deferral, palette→sheet focus ownership via shell lock + handingOff flag. Tests `navigationCounts` 10 pass |
| D7 | ✅ `useFocusTrap`, Escape closes, return focus to opener, palette→sheet handoff, RTL `dir=rtl`, `prefers-reduced-motion`. Tests `commandPaletteFocusReturn` 4 pass, `a11yGate` 14 pass |
| M-2 | ✅ `Teacher.photoMediaId`, Avatar via `useMediaObjectUrl`, existing Media/ProfilePhotoField. Tests `teacherPhoto` 8 pass |
| M-3 | ✅ `Student.photoMediaId`, same mechanism, detail surface. Tests `studentPhotoDisplay` 6 pass |
| M-4 | ✅ I15 failure disclosure: ErrorState + retry, not EmptyState/zero, legitimate empty still empty. Tests `failureDisclosureI15` 24 pass, `metricsFailureDisclosure` 15 pass, no catch→[] |
| M-5 | ✅ I16 deep-link independent of 200-row ceiling: `useStudent`/`useTeacher` `get(id)` authoritative, beyond 200 resolves, list unchanged per_page 200, loading/error/not-found distinct. Tests `deepLinkPagination` 11 pass. Residual Classes `find` noted as deferred |
| M-6 | ✅ Settings honesty: Toggle disabled, explicit deferral Surfaces notifications (2), localization (1), session rules (1), working hours expanded, disabled 7 notification toggoles, 6 localization selects, 4 session-rules inputs, Friday toggle disabled "— غیرفعال". Tests `settingsHonesty` 14 pass |
| D9 | ✅ Build 2037 modules, `index.css` 112.39kB gzip 16.39kB, `operationsViews` 161.76kB gzip 47.07kB, `vendor` 192.35kB gzip 60.29kB, `academicViews` 201.42kB gzip 47.86kB, `index` 409.71kB gzip 118.50kB, entry gzip 118.50kB <273791, total ~300kB within +5% cap 305805, no chunk ≥400k, built 4-5s |
| Domain architecture | ✅ No fixture imports in views/components (only comments), no hardcoded factual product state, no fake persistence (localStorage only via demoStore single authority + recentTargets UX + auth), no hardcoded success without write, registry 7 mode-switched + 11 demo-in-both-modes with `DEMO_SERVED_DOMAINS` |
| Typecheck | ✅ `npx tsc --noEmit --skipLibCheck` 0 errors |
| Full suite known failure | ✅ 150 passed 1 failed files, 2102 passed 10 failed tests (2112 total) — 10 failures only `src/__tests__/projectState.test.ts` SHA/branch expectations, known historical, not new, test not altered |
| Browser QA | ❌ **NOT VERIFIED** — formal final-audit disposition, no claim of complete browser verification, external-QA only |
| Deferred | ✅ Classes deep-link beyond capped list, Finance/Reports backend/domain, notification server wiring, localization/settings server wiring, working-hours/session-rules server wiring, free-slot search, external browser QA — **deferred items do NOT modify frozen frontend** |
| Backend boundary | ✅ Frontend freeze precedes Laravel/backend implementation |
| Canonical policy | ✅ All future frontend fixes require explicit decision to reopen freeze |
| Docs only | ✅ No production code/tests changed, only `docs/engineering/` |

### M10 validation (measured at `e7a64b7777be36ddd97fc3337898fad794118e5b`, the milestone's single implementation checkpoint)
...
| `git diff --check` | ✅ PASS |
| New boundary gate (`src/__tests__/m10Boundary.test.ts`) | ✅ 13/13 — dissolution, import ban (incl. its own probe-string-only self-exemption), seed-import ban, retired-symbol tombstones, deferred-surface honesty, single-owner map, sanction list; every ban replayed against a synthetic violation in-file |
| Protected F2/I8 family (compensation, attendance, scheduling suites) | ✅ PASS — 315 passing in the family run; zero diff in `src/domains/compensation/**`, `src/domains/attendance/**` and every persistence/repository file (no schema, no ID, no collection semantics, no backup-envelope change) |
| Fixture-era gates, mechanically re-derived (relationsNoFixtures, messagesNoFixtures, schedulingNoFixtures, attendanceNoFixtures, compensationNoFixtures, architectureBoundaries, noSuccessWithoutWrite, Library.test) | ✅ PASS — same rule, relocated target; `it(` counts identical before/after in all eight; no assertion skipped, broadened or weakened |
| Mutation/liveness probes on the new gates | ✅ 6 probes — retired-symbol revival (production and at depth), duplicate single-owner export, deferred-surface marker removal (fails both the gate and the honesty suite), seed import in a view, dissolved-module import revival (fails two independent gates) — each fired against a mutated tree and each revert verified byte-identical |
| Full test suite at the checkpoint | **1988 passed / 11 failed / 8 skipped (1999 tests)** — 10 failures are the **pre-existing** `src/__tests__/projectState.test.ts` checkpoint-object failures of this squashed clone (the recorded Phase-checkpoint SHAs and quoted historical SHAs do not exist as objects here; reproduced identically on the pristine pre-M10 checkpoint `98af31b`; the test file itself was touched by **no** M10 commit — diff is zero), and 1 was the **expected** stale-documentation-reference failure the dissolution necessarily introduced — reconciled by this documentation pass; a further 2 checkpoint-object checks were flippable by *truthful registration alone* (see below) |

**Post-closure accounting (this pass, documents only).** The stale-path sweep makes the 60 missing
references point at real files, so the stale-path check passes. Two further checks flip without
touching the test: *the recorded working branch* (now the actual branch, §2) and *the recorded
documentation checkpoint really carries documentation* (the docs checkpoint registered by this pass is
`98af31b`, a real commit carrying documents). The remaining `projectState` failures are the **same
class recorded at M7's item 18**: quoted pre-squash SHAs (`33b1031…`, `aca40c5…`, `3a16549…`,
`8d34eb3…` and the older phase-ledger quotes) name commits that do not exist as objects in this
squashed clone. They are preserved **as known environmental failures**, exactly as instructed — the
checkpoint history is not rewritten and `projectState.test.ts` is not weakened to hide them.

### M9 validation (measured at `8d34eb3d1cd639cffc794596250c897b5ed4b6b3`, the milestone's single implementation checkpoint)

M9 landed in **one** product commit on top of `c186baaa1dda39e274369ed0387094822b00f0ae` — **M8's
documentation closure**, the twenty-seventh documentation checkpoint, which is therefore M9's effective
safe rollback boundary. The numbers below were measured on that tree:

| Check | Result |
|---|---|
| Starting point | ✅ `git rev-parse HEAD` == `c186baaa1dda39e274369ed0387094822b00f0ae`, `git status --porcelain` empty, `git ls-remote` returning the same SHA, and the clone **repaired to full** first — the provisioned checkout arrived **shallow at the fork point again** (`git rev-list --count HEAD` = 1, `.git/shallow` present), so `git fetch --unshallow` ran before the branch was re-attached from the authoritative remote with no reset, no rebase and no history rewrite |
| Baseline first | ✅ `npm ci` (176 packages), then `npm run typecheck` (clean), `npm run build` (3.78 s), `git diff --check` (clean) and the full suite — **1 923 passed / 2 failed of 1 925**: the recorded environmental branch-name case (§7 item 16) and the date-dependent `schedulingWrites` case (§7 item 18) |
| What M9 changed | ✅ 14 files, **3 025 insertions / 328 deletions** — eight modified (`src/components/ds/primitives.tsx`, `src/components/ds/blocks.tsx`, `src/components/panels/Signals.tsx`, `Intelligence.tsx`, `AttentionAndFlow.tsx`, `BusinessIntelligence.tsx`, src/data/academy.ts (the design-time fixture module, dissolved at M10) — the `Signal` type only, `src/views/Dashboard.tsx`) and six new (`src/domains/shared/dashboardInsights.ts`, `src/domains/shared/useDashboardInsights.ts` and four test files). **`git diff --name-only c186baa..8d34eb3 -- src/services src/api src/context src/domains/auth src/domains/demo docs` prints nothing**, and no existing test, gate or config file was edited |
| Dependencies | ✅ none added, removed or changed — `package.json` and the lockfile are absent from the diff |
| I9 first, and it is guarded | ✅ the empty-series guard landed **before** the live data could reach it, and it is pinned by behaviour, not by a comment: `src/components/ds/__tests__/seriesGuards.test.tsx` (**7 cases**) drives `Sparkline` with `data={[]}` and `data={null}` in both of its kinds and the signals row over an empty record set, asserting the rendered no-value glyph. Removing the guard mid-milestone failed **5** of those cases with `Cannot read properties of undefined (reading 'x')` — the same class of failure the original code would have produced — and the guard was restored byte-identically |
| H4 — every figure traced | ✅ the at-risk count and mean attendance from student rows (`sessionsTotal > 0` filter, not the P1-capped hero metric); sessions/cancellations and their weekly series from the bounded `useSessions` window; overdue payments and waitlists from students and classes; today's rows, their class/room/teacher labels and the seeded room clash from the sessions/classes/rooms/teachers reads; receivables from `Student.balance`; roster growth from `Student.since`; occupancy from class seats and stored weekdays; instrument mix from `Student.instrument`; record counts from the entity totals. Fixture sources removed from those surfaces: `signals`, `growthSeries`, `revenueSeries`, `occupancy`, `instruments`, `quickActions`, `todayFlowIds`, `attentionItems`, `attentionQueue`, `intelligenceCards` |
| The seam that was missing | ⚠️ **collected revenue has no authoritative source** — Finance and Reports are README-only (`src/domains/finance/README.md`, `src/domains/reports/README.md`, deferred by **D6**/**I2**) and `invoices`/`payments` have **no repository**, so the revenue chart was **removed** rather than recomputed and the money slot now shows live receivables. **No Finance domain, no repository and no fabricated revenue figure were created**; the reason is recorded in the code (`src/components/panels/BusinessIntelligence.tsx` header, `src/views/Dashboard.tsx`) and here. This is **not** Finance/Reports implementation |
| New suite | ✅ **61/61** in four new files — `seriesGuards.test.tsx` (7), `src/domains/shared/__tests__/dashboardInsights.test.ts` (35, the derivations against hand-built record sets including every empty-input case), `src/components/panels/__tests__/panelsEmpty.test.tsx` (12, the six panels over an empty record set) and `src/views/__tests__/dashboardInsightsLive.test.tsx` (7, the real `Dashboard` over DEMO and EMPTY) |
| Focused batch | ✅ **164 passed** across eleven suites — the four new files above plus `architectureBoundaries` (11), `privacyPosture` (22), `cspCompatibility` (17, against a **real `dist/`**), `navigationCounts` (7), `emptyEnvironmentPanels` (10), `emptyEnvironment` (22) and `attendanceMetric` (14, the P1 hero guard) |
| Full suite | ✅ **1 985 passed / 1 failed of 1 986** — 61 new cases over the 1 924-case baseline, and the single failure is the recorded environmental branch-name case (§7 item 16). **The date-dependent `schedulingWrites` case (§7 item 18) passed in this run; that does not resolve it** — it depends on which weekday the sandbox clock reaches, and it is neither modified nor weakened |
| `npm run typecheck` | ✅ clean (zero output, exit 0) |
| `npm run build` | ✅ exit 0, **3.57 s** on a **fresh** `dist/` (the pre-existing chunk-size warning is unchanged — **I6**), and the fresh build is what `cspCompatibility` parsed |
| `git diff --check` | ✅ clean |
| EMPTY, asserted over the real tree | ✅ no fabricated figure and no fabricated sentence: the panels render, each states «داده‌ای نیست», the signals strip carries exactly two `NO_DATA` glyphs (the two measures with no stored history — the two weekly counts are *measured* zeros and draw a flat line), **no `NaN`/`Infinity` appears in text or in any SVG attribute**, and none of the 24 retired fixture sentences or 5 retired fixture figures survives. **The EMPTY dataset was not modified to pass** |
| Mutation / liveness probes | ⚠️ the implementer's own, each restored byte-identically (`sha256sum -c` against recorded baselines): removing the Sparkline guard → **5** failures; removing a chart's empty-set guard → **6** failures including the original `Cannot read properties of null (reading 'value')`; hard-coding a retired figure back into the derivation → **4** failures across two suites, in **both** DEMO and EMPTY; restoring the hardcoded pulse kicker → 2 failures; dropping `per_page` from a paged read inside the new hook → the page-size gate, naming the offending file. Two probes that are not mutations: **fixture-value substitution** — every retired fixture value substituted with an absurd one left all **82** cases across the four M9 suites and the two gate suites it drives green, which is the point (the fixtures are inert on those surfaces) — and **liveness**, a real write through `getStudentRepository().update()` moving a figure the panel displays. **No independent audit re-measured any of them:** M9 carries no acceptance audit and therefore no **B**-numbered coverage items |
| Browser QA | ❌ **NOT VERIFIED** (§5) — the panels, the charts and the derived copy are jsdom-verified only; no browser pass ran and none is claimed |
| What the closure does not claim | ❌ **not every fixture renderer is gone**: `src/views/DesignSystemView.tsx` still renders fixture *samples* (`signals`, `intelligenceCards`, `attentionItems`, `schedule`, `insights`) as component demonstrations, `src/views/Finance.tsx` and `src/views/Reports.tsx` remain fixture-driven (**I2**, deferred by **D6**), and `src/components/hero/Hero.tsx` plus `src/domains/demo/seed.ts` keep the fixture identity recorded at §7 item 20. Fixture **separation** is **D5**/**M10**'s, not M9's. The read set states `per_page: 500` on all five reads and does **not** distinguish a partial page from a complete one — the **I16** ceiling pattern, inherited rather than changed. And only the four panels plus the dashboard were rewired: no other surface was touched |
| Remote | ✅ pushed as a fast-forward (`c186baa..8d34eb3`) and verified — `git fetch` + `git ls-remote` both returned `8d34eb3d1cd639cffc794596250c897b5ed4b6b3`, with `git rev-list --count FETCH_HEAD..HEAD` = 0 and a clean worktree |
...
| What the closure does not claim | ❌ **the fixture identity is not gone from the product**, only from the two surfaces M8 was authorized to rewire: `src/components/hero/Hero.tsx` still renders `academy.name` and `src/domains/demo/seed.ts` still derives the demo organization settings from src/data/academy.ts (the design-time fixture module, dissolved at M10) — outside M8's scope by the milestone's own out-of-scope clause, recorded at §7 item 20 and left for M9/M10. Branding still resolves to Demo in both modes (**D8**), the logo/favicon upload redesign stays out of scope, and no export path was audited for the academy name |
...
| Domain / contract scope | ✅ **no product change anywhere under `src/domains/`, `src/services/`, `src/api/` or `src/context/`**: the only non-view product files in the diff are src/data/academy.ts (the design-time fixture module, dissolved at M10) and `src/components/layout/Sidebar.tsx`. No backend contract, no new repository verb, no new API endpoint, no seed or fixture change, and no dependency |
| Scope of the diffs | ✅ per checkpoint — CP1 **4 files, 1 073/0** (gate, plumbing suite, two new modules), CP2 **3 files, 899/85**, CP3 **3 files, 1 330/134**, CP4 **6 files, 1 606/168**. Whole milestone (`8131c5c..f1ec0dd`) = **13 files, 4 842/321**: 3 views, src/data/academy.ts (the design-time fixture module, dissolved at M10), `src/components/layout/Sidebar.tsx`, 2 new modules and 6 new test files. `git diff --check` clean at every checkpoint |
...
| 19 | Spec clause: *"views do not import src/data/records.ts (the design-time fixture module, dissolved at M10) **for data**"* | **PARTIAL — and the boundary is stated** | the three rewired views import **labels and types only** (`WEEKDAYS`, `WEEKDAYS_SHORT`, status/label maps, `AcademyClass`, `InstrumentId`), which the gate's allow-list permits and which carry no relation or row data. The spec's M7 scope also named `Messages.tsx`, `Library.tsx` and `Settings.tsx`; **those were not in the authorized checkpoints and are untouched** — they still read `messageTemplates`, `libraryShelves` and `settingsSections`, and the product-wide boundary test the final-phase DoD asks for remains **M10/D5** |
| 20 | Spec clause: *"EMPTY renders «داده‌ای نیست» everywhere these relations appear"* | **PASS at the surfaces that changed** | `emptyEnvironment.test.tsx` and `emptyEnvironmentPanels.test.tsx` are untouched and green; the M7 suites' own empty/unresolvable/partial cases assert «—» and explicit empty copy rather than zeros. The dashboard panels' fabricated sentences (the other half of **I1**'s neighbourhood) were **M9/H4** at the time this row was measured, and **closed at M9** (`8d34eb3`, 2026-09-16) — the row is kept as measured rather than rewritten |
...
M10 changed the Hero, the command palette's free-class answer, the Finance/Reports deferred surfaces
and several design-system catalogue entries; none of them has been browser-verified either — the same
jsdom + `tsc` + build evidence class as everything above. **The M10 closure itself is documents-only**
and changes no renderable surface at all.
**M11's L4 environment decision (recorded 2026-09-16, documents only):** this environment **cannot perform the browser-QA checklist** — inspection of `package.json` and `package-lock.json` shows no Playwright, Puppeteer, Cypress or WebDriver dependency, no browser engine is installed, and installing one is both outside the authorized scope and forbidden by the no-new-dependency rule (§10). The specification's §M11 acceptance clause permits exactly two dispositions — QA performed and recorded, or **NOT VERIFIED** — so the standing disposition is: **the checklist stays NOT VERIFIED here**, and may only graduate in a separate, externally-capable QA environment whose use is itself authorized by the owner. Nothing in M11 may report browser verification without that environment's evidence attached.

**M11's landing does not change this paragraph (checked again at the closure pass, 2026-09-17):** the tooling census was re-run, nothing was installed, the checklist did not run, and **L4 stays NOT VERIFIED exactly as the spec's disposition allows**. M11 adds no surface to the *already-open* manual-QA checklist except one measured-behaviour note ('§7 item 23'): the lazy workspace chunks' first fetch on navigation is jsdom-verified only.

**Arena Frontend Freeze — 2026-09-19, frozen HEAD `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb` on `arena/01a0b6be-parsian-music-dashboard-opus` — formal final-audit disposition:** pre-freeze read-only audit inspected Students/Teachers deep-link via `useStudent`/`useTeacher` `get(id)` authoritative fallback, Dashboard single ErrorState honesty, Settings M-6 functional/demo-local/unimplemented classification, D7 focus-trap/RTL/reduced-motion, D9 build entry 118.5k gzip <273791 total ~300k within 305805 cap, compensation lineage safety, no fixture/hardcoded/fake patterns; **browser QA NOT VERIFIED** — this environment has no Playwright/Puppeteer/Cypress/WebDriver, `package.json` grep 0, no browser binaries, all verification jsdom + `tsc` + Vite build only, **no claim of complete browser verification** and **no claim that backend functionality already exists**; external browser QA required in authorized environment. **M-1..M-6, D7, D9 CLOSED** at this checkpoint, deferred items explicitly do NOT modify frozen frontend, backend boundary frontend precedes Laravel/backend, canonical policy future fixes require explicit reopen decision. **Validation: full suite 2102 passed / 10 failed — 10 failures known historical `projectState.test.ts` SHA/branch expectations, test not altered.**
...
| These engineering documents stay truthful | `src/__tests__/projectState.test.ts` — the preserved-branch SHA exception and the durable working-branch check are recorded in [DECISIONS.md](DECISIONS.md) §21 |
| **The performance budget, re-baselined after D9-A-lite** (the total gzip baseline is the post-reduction measurement and its +5% cap follows from it; the entry anchor and the 400000 B gzip ceiling are unchanged) | `src/__tests__/bundleBudget.test.ts` — the measurement, the re-baseline and the accepted A2 evidence are recorded in [DECISIONS.md](DECISIONS.md) §22 |
...
3. **Views not wired to their real domains — both down, and the two that remain have no domain at
   ...
   **I2**'s (deferred by **D6**) rather than a wiring milestone's — and **M9 landed without creating
   either domain**, rebuilding its own money slot from the student records instead. What the two completed wirings do *not* cover is
...
6. ~~**Latent DS crashes**: `Sparkline` with `data={[]}` and `BusinessIntelligence` with an empty
   ...
   **CLOSED at M9** (`8d34eb3d1cd639cffc794596250c897b5ed4b6b3`, **I9**, landed **before** H4 so the
   crash could not be reached by live data): `Sparkline` takes `readonly number[] | null`, `Delta` takes
   `number | null`, and every chart guards its own empty model before indexing — all of them rendering
   the product's typed no-value glyph (`NO_DATA`, DECISIONS.md §14). Pinned by
   `src/components/ds/__tests__/seriesGuards.test.tsx` and `panelsEmpty.test.tsx`.
7. **Command-palette natural-language matching is substring-loose** (`q.includes(keyword)`).
8. **Dead code**: `TeacherNote` in src/data/records.ts (the design-time fixture module, dissolved at M10) is defined and never used.
...
16. **One test has failed in every session of this milestone, and it is environmental — it is recorded,
    ...
    *(Superseded 2026-09-25: the assertion this item measured is replaced by a session-independent
    durable check — the recorded branch must exist on the remote and stay level with it — by
    recorded owner decision ([DECISIONS.md](DECISIONS.md) §21). This item remains the record of the
    period and of the sandbox-branch facts it measured.)*

17. **M7's own limitations, none of them closed by declaring the milestone complete.** **The fixtures are not deleted, and their three roles are untouched** — M7 removed *readers*, not data: src/data/records.ts (the design-time fixture module, dissolved at M10) and src/data/academy.ts (the design-time fixture module, dissolved at M10) still provide entity types, the canonical DEMO seed (`src/domains/demo/seed.ts`) and the fake data other views still render, and the separation is **M10/D5**. *(Correction, 2026-09-16, M10 `e7a64b7`: this first clause is **closed** — the two fixture modules are dissolved, the canonical DEMO seed lives at `src/domains/demo/academySeed.ts`, the entity types are domain-owned, and the fake data is removed and tombstoned; src/data/records.ts (the design-time fixture module, dissolved at M10) and src/data/academy.ts (the design-time fixture module, dissolved at M10) below name the dissolved design-time fixture modules as they stood at M7, kept for provenance.)* **`useStudentList` is still un-keyed** (**I13**, above): M7 reached it from three surfaces and guarded each mount with the consumer-side key the gate now requires, which is a mitigation and not a closure. **The `per_page: 200` ceilings stay** (**I16**): every M7 read states one and every surface distinguishes a partial page from a complete one in its own words, but the ceiling itself is unchanged and no other consumer was touched. **Three views the milestone's original scope named were not in the authorized checkpoints** — `src/views/Messages.tsx` (`messageTemplates`), `src/views/Library.tsx` (`libraryShelves`) and `src/views/Settings.tsx` (`settingsSections`) still import fixture *content*, and the product-wide boundary test the final-phase DoD asks for does not exist yet (**M10**); the same is true of the dashboard insight panels (`src/components/panels/`), which are **M9/H4**. **The command palette's substring matching is untouched** (**L1**): M7 removed the false *numbers* from the hints, not the loose matching behind them. *(Correction, 2026-09-16: the fourth surface this item named — the dashboard insight panels — came off the list at **M9** (`8d34eb3`), which rebuilt them on the environment's own records; the three views and the gallery's fixture samples above remain.)* **No acceptance audit ran** for M7, so it carries no audit findings and no **B**-numbered coverage items — the reversion and mutation checks in §4 are the implementer's own measurement. And **browser QA has never run** (§5): the three rewired surfaces and the badge are jsdom-verified only, and no test unmounts and remounts a surface, so "survives a reload" rests on the demo store's single-persistence-authority code path exactly as M3, M5 and M6 recorded for theirs.
...
20. **M8's own limitations, none of them closed by declaring the milestone complete.** **The fixture
    ...
    derives the demo organization settings from src/data/academy.ts (the design-time fixture module, dissolved at M10) *(Correction, 2026-09-16, M10
    `e7a64b7`: both are **closed** — the Hero renders the M8 branding record and the signed-in
    operator's name through `useAuth`, and the seed derives from `src/domains/demo/academySeed.ts`; the
    src/data/academy.ts (the design-time fixture module, dissolved at M10) mention below names the dissolved fixture module as it stood at M8, kept for
    provenance)*; that is outside the milestone's
    own **Out of scope** clause (it named the shell and the login screen, not the whole product), it is
    not hidden by the acceptance record, and it stays with **M10/D5** — **M9** was authorized afterwards
    and landed without touching it either (its scope was the dashboard, not the hero or the seed; see
    §7 item 21). **Branding still
...
21. **M9's own limitations, none of them closed by declaring the milestone complete.** **The revenue
    measure has no seam and was therefore deleted rather than computed:** collected revenue needs the
    Finance/Reports domains (**D6**, **I2** — still planned, still absent), so `BusinessIntelligence`'s
    revenue chart is **gone** and the money slot shows the receivables the student records carry. No
    Finance domain, no `invoices`/`payments` repository and no revenue figure were invented — but a
    surface that existed before is now absent, which is a visible change and is recorded as one.
    **Fixture renderers elsewhere survive:** `src/views/DesignSystemView.tsx` still renders fixture
    samples (`signals`, `intelligenceCards`, `attentionItems`, `schedule`, `insights`) as component
    demonstrations, and `src/views/Finance.tsx` / `src/views/Reports.tsx` remain fixture-driven
    (**I2**) — the separation of the fixtures' three roles is **D5**, recorded for **M10**, and M9
    deliberately did not start it. **The hero and the demo seed are untouched** (see item 20):
    `src/components/hero/Hero.tsx` still prints `academy.statusLine` beside a hardcoded «وضعیت: سالم»
    badge, and `src/domains/demo/seed.ts` still derives the demo organization settings from
    src/data/academy.ts (the design-time fixture module, dissolved at M10) — outside M9's authorized scope by the milestone's own out-of-scope clause.
    **The read set is ceiling-bounded and says so:** all five reads state `per_page: 500` (the page-size
    gate's rule) and none distinguishes a partial page from a complete one (**I16**'s pattern,
    inherited, not changed); a stored set larger than one page would be under-counted rather than
    announced as partial. **`Sparkline`'s prop widened** from `number[]` to `readonly number[] | null`
    and `Delta`'s from `number` to `number | null` — a design-system contract widening every existing
    caller inherits, deliberate (null means "no comparison", not zero) and pinned by the new guard
    suite. **No acceptance audit ran** for M9, so it carries no audit findings and no **B**-numbered
    coverage items; the probes in §4 → "M9 validation" are the implementer's own measurement, and the
    `schedulingWrites` case that passed in this milestone's run is **not** resolved by it (item 18).
    And **browser QA has never run** (§5): the dashboard, its four panels, the charts and the derived
    copy are jsdom-verified only.
...
22. **M10's own limitations and the two implementation-report errata, none of them hidden by declaring
    the milestone complete.** **No free-slot search was built:** the Tuesday free-class question keeps
    its honest no-data answer; a real availability read remains future work with no owner yet.
    **Finance and Reports are NOT implemented** — they are explicitly deferred surfaces (**D6**,
    [OPEN_ITEMS.md](OPEN_ITEMS.md) **I2**) whose deferral is visible on screen and mechanically
    enforced (`src/__tests__/m10Boundary.test.ts`), and every fabricated finance/reports measurement is
    removed, not relocated. **The design-system catalogue keeps plated configuration**
    (`src/components/ds/samples.ts` — a palette of sample values for catalogue pages, not a data
    source). **No acceptance audit ran for M10:** the mutation probes in §4 → "M10 validation" are the
    implementer's own measurement, not an external audit. **Browser QA has not run** (§5) for the Hero,
    palette, deferred surfaces or catalogue changes. **The squashed-clone checkpoint failures of
    `src/__tests__/projectState.test.ts` are preserved as known environmental failures** (§4), exactly
    as at M7 item 18 — checkpoint history is not rewritten and the test is not weakened.
    **`docs/architecture/demo-data.md` line ~147 still names the src/data/ directory as the hand-authored source
    the demo dataset is derived from — a strict implementation contradiction left deliberately
    unfixed:** that file is outside the five authorized drift targets, the scope rule says STOP and
    report rather than expand, and it is forwarded to the next documentation pass with owner
    authorization (also recorded in [OPEN_ITEMS.md](OPEN_ITEMS.md) **L7**).
    **ERRATUM A — `ClassSession.resourceId`.** A previous implementation report claimed M10 *removed*
    `ClassSession.resourceId`. **This is false:** the forensic audit established the field occurs
    **zero** times in the pre-M10 checkpoint and **zero** times post-M10, and no runtime or demo record
    ever contained it. The actual M10 change to `ClassSession` was a **type-ownership relocation**
    (to `src/domains/scheduling/types.ts`) and a **D5 disambiguation rename** of its status vocabulary to
    `ClassSessionStatus` — **no runtime or data-model field was removed**, and M10 documentation must
    not describe one. **ERRATUM B — demoStore R4/R6.** The same report overstated new `demoStore`
    lifecycle protections. **The audit established the opposite:** `demoStore` (`src/services/demoStore.ts`)
    changed by **one hunk** — a type-import re-point (`Student` → `src/domains/students/types.ts`) —
    and the lifecycle machinery it references (environment marker, adoption-as-DEMO, never-fill-EMPTY,
    `src/domains/demo/lifecycle.ts`, `demoDataManager.ts` and the unchanged
    `dataLifecycle.test.ts`) is **pre-existing**, not an M10 implementation; M10 documentation must
    describe R4/R6 lifecycle behaviour as pre-existing machinery.
    **Branch-name errata (documented, not repaired):** an earlier note designated
    arena/01a0a9b6-parsian-music-dashboard-opus as the implementation branch; **that branch does not
    exist** in this repository (verified against origin and every local ref). The actual implementation
    branch is arena/01a0aa83-parsian-music-dashboard-opus (§2), carrying `e7a64b7` on parent
    `98af31b`; no rename, merge, cherry-pick, reset, rebase, force-push or other branch repair was
    performed, and none is planned here.
23. **M11's own limitations, none of them hidden by declaring the milestone complete.**
    **Lazy-loading first-paint residual:** the academic and operations workspace chunks (~50 and
    ~46 kB gzip at landing) are fetched on first navigation, so first-paint is fast but the
    *first* visit to a non-eager view pays that fetch — jsdom-verified, never browser-measured;
    when L4's browser checklist is ever run in an authorized external QA environment, first-view
    navigation timing belongs in it. **No acceptance audit ran for M11:** the mutation/liveness
    evidence (restore-by-revert probes in the D8/D9/D7 suites) is the implementer's own. **No
    performance benchmark beyond bytes:** D9 is a size budget — runtime render timings are not
    measured anywhere. **The environmental `projectState` failures are preserved as known
    environmental checkpoints-class failures** (§4 → "M11 validation"), exactly as at M7 item 18
    and M10 item 22 — history is not rewritten and the test is not weakened. And the sandbox
    **re-clone mid-pass is disclosed**: the workspace reverted to a squashed re-clone between
    turns; the branch was re-attached read-only at the pushed `f7eb867` with the tree verified
    byte-identical to the pushed state — no commit, no ref rewrite, no force-push — and `npm ci`
    was re-run because `node_modules` was gone (restoration, not a dependency change).

24. **Arena Frontend Freeze — 2026-09-19 — frozen at `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb` on `arena/01a0b6be-parsian-music-dashboard-opus` — formal closure limitations.** **M-1..M-6, D7, D9 CLOSED** with measured evidence (§4 → "Arena Frontend Freeze validation"): M-1 real dialogs, M-2/M-3 photoMediaId, M-4 ErrorState+retry, M-5 `get(id)` deep-link, M-6 disabled controls + Surfaces, D7 focus-trap/RTL/reduced-motion, D9 118.5k entry <273791 total ~300k within 305805. **Browser QA NOT VERIFIED** as formal final-audit disposition — no claim of complete browser verification, no claim backend already exists, external QA required. **Deferred explicitly do NOT modify frozen frontend:** Classes deep-link beyond capped list (`src/views/Classes.tsx:678` capped `find` without `get`), Finance/Reports backend/domain implementation (I2/D6), notification server wiring, localization/settings server wiring, working-hours/session-rules server wiring, free-slot search, external browser QA. **Backend boundary:** frontend freeze precedes Laravel/backend implementation, api mode seam only. **Canonical branch policy:** all future frontend fixes require explicit decision to reopen freeze. **Known full-suite failure:** `src/__tests__/projectState.test.ts` 10 SHA/branch expectation failures, historical environmental, test not altered. **Preserved history:** M11 chain, M10 chain, working branch history, other branches untouched.
...
was dropped. **For the Arena Frontend Freeze (2026-09-19, `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb`): deferred items explicitly do NOT modify the frozen frontend — Classes deep-link beyond capped list, Finance/Reports backend/domain, notification server wiring, localization/settings server wiring, working-hours/session-rules server wiring, free-slot search, external browser QA. Backend boundary: frontend freeze precedes Laravel/backend. Canonical policy: future fixes require explicit reopen decision. No claim of complete browser verification, no claim backend exists.**

## 9. Immediate next action

**Arena Frontend Freeze — 2026-09-19 — ✅ COMPLETE at `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb` on canonical `arena/01a0b6be-parsian-music-dashboard-opus` — documentation-only closure.** M-1 Quick Actions CLOSED, D7 Accessibility CLOSED, M-2 Teacher Photo CLOSED, M-3 Student Photo CLOSED, M-4 I15 Failure Disclosure CLOSED, M-5 I16 Deep Links CLOSED, M-6 Settings Honesty CLOSED, D9 Performance CLOSED. Frozen HEAD includes M11 (four checkpoints ending at `f7eb867` on `3ed0bfe`) and M10 (`e7a64b7` on `98af31b`) preserved. **No further frontend work authorized until explicit decision to reopen freeze.** Backend boundary: frontend freeze precedes Laravel/backend implementation. Canonical branch policy: all future frontend fixes require explicit decision to reopen freeze. Deferred items do NOT modify frozen frontend: Classes deep-link beyond capped list, Finance/Reports backend/domain, notification server wiring, localization/settings server wiring, working-hours/session-rules server wiring, free-slot search, external browser QA. Browser QA NOT VERIFIED as formal final-audit disposition, no claim of complete browser verification, no claim backend already exists. Known full-suite failure `projectState.test.ts` 10 SHA/branch expectations preserved, test not altered.

**M10 and M11 were both ✅ COMPLETE (M10 on 2026-09-16 at `e7a64b7777be36ddd97fc3337898fad794118e5b`; M11 on 2026-09-17, four checkpoints ending at `f7eb86762dc11f7d5dd3c7218e7c697fad29488e`) — and the phase specification names no milestone beyond M11, so the next phase was NOT STARTED and NOT AUTHORIZED before the freeze.** The
paragraph below is the M9 closure pass's own lead, kept as written. **M9 is ✅ COMPLETE (2026-09-16) at `8d34eb3d1cd639cffc794596250c897b5ed4b6b3`, and M10 is not
authorized.** The owner authorized M9 on 2026-09-16 and it landed in **one** implementation checkpoint,
built on **M8's documentation closure** `c186baaa1dda39e274369ed0387094822b00f0ae` — so `c186baa` is M9's
effective safe rollback boundary — with its measured evidence in §4 → "M9 validation" and its
limitations at §7 item 21. It closed **I9** first and **H4** after it, in that order, exactly as
[PRODUCT_PHASE_SPECIFICATION.md](PRODUCT_PHASE_SPECIFICATION.md) §5's M9 block required; **the one thing
...
**The M11 closure pass (2026-09-17) is documents-only, and no next work is authorized.** It registers
M11 ✅ **COMPLETE** against its measured evidence (§4 → "M11 validation"): the four implementation
checkpoints `3eed4f19` → `24998d83` → `53794d57` → `f7eb86762` built on the D7/D8/D9 decision record
`3ed0bfe4ac928451a4bddf9de3a34f7be681c33f` — M11's effective safe rollback boundary — the split whose
warning went silent by real splitting, the **D8** disclosure on the registry's single
`DEMO_SERVED_DOMAINS` enumeration, the 14-case **D7** accessibility gate and the final **D9** values
(entry gzip 108 207 B, vendor 60 108 B, total 298 249 B, largest chunk 108 207 B, `dist/` raw
1 964 789 B). It records the implementation report's "starting SHA" erratum (**starting SHA =
`3ed0bfe`; final implementation HEAD = `f7eb867` — the commits are untouched) and the seven `projectState`
failures as the known environmental checkpoint-object class — nothing reconditioned, nothing weakened.
It preserves **L4 = NOT VERIFIED** (§5) and the lazy-loading first-paint residual (§7 item 23). It
changes **no** product source, test, gate, dependency, config, seed or backend; it modifies no Git ref
and does not self-register its own SHA (§2 — this pass's commit is the **thirty-second** documentation
checkpoint by this ledger's count and is named by whichever documentation commit comes next);
**L2, L7, I2, I19** and every other recorded residual stay open; and the phase specification names no
milestone beyond M11 — **M12 is NOT STARTED / NOT AUTHORIZED** until the owner says otherwise.
...
remains ✅ COMPLETE**, **M8 is ✅ COMPLETE at `735617d0324a8f4ba2e243846eedf069d389751a`** (authorized by
the owner, built on `a039ab0`), and **M9 is ✅ COMPLETE at
`8d34eb3d1cd639cffc794596250c897b5ed4b6b3`** (authorized by the owner on 2026-09-16, built on M8's
documentation closure `c186baa`, **I9** first and **H4** after it) — so **M10 (fixture / type / seed
separation, D5) and M11 are ❌ NOT STARTED and not authorized**.**
...
authorized afterwards and has since landed at `735617d` on the pre-M8 boundary `a039ab0`. **Correction
(2026-09-16, M9's documentation closure):** M9 was authorized after that and has also landed, at
`8d34eb3` on M9's own boundary `c186baa`, so the contract is unchanged and now applies to
**M10, which is not authorized** — and the scope it points at is M10's, the milestone after M9 in §4's
