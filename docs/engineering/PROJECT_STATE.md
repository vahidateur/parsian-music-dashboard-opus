...
- A **documentation checkpoint** is a pushed commit that changes documents and validation gates
  but no product behaviour. **Twenty-four** are **named** so far — the twenty-fifth is the commit carrying
  ...
  registration**, named by the commit after it — the same rule again). They are listed in
  [PHASES.md](PHASES.md) → "Documentation checkpoints" so that `git log` never
