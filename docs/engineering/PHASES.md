...
| M10 — fixture / type / seed separation | the separation shape recorded as the seven **D5** principles **and the same-day D5 follow-ups (DEMO seed, legacy sessions/attendance, Hero, Finance/Reports)**, the documentation-drift table, **L2** | ❌ **not started — not authorized** (its first dependency — **D5 recorded before execution** — was **discharged 2026-09-16** by the owner's D5 record in [DECISIONS.md](DECISIONS.md) §19, which **does not authorize M10**; **L3** is already closed — 2026-09-14 — and is no longer M10's scope) |
