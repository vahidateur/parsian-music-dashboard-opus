...
| Product-feature phase — M7 (relation de-fixturing + sidebar badges, **I1**) | `f1ec0ddde783aec14d6429ac2457f085f851ad9a` | ✅ yes (pushed) | ✅ **COMPLETE** — four implementation checkpoints built on M6's coverage-matrix commit `8131c5cd49c35a7163543082475a9635914aba40`: CP1 `0d9fc01079039548cf0ffe3c80c5bf2e88056a9c` (relation plumbing — the academy-day and id-index modules, the per-surface fixture gate, the through-route suite), CP2 `5726435506bd74748f6165819f3c80e30f359b80` (student relations), CP3 `ede3ad6a5e26b1cdfc514962b57ac1302309968c` (teacher relations) and CP4 `f1ec0ddde783aec14d6429ac2457f085f851ad9a` (class relations + navigation counts) — **67 new tests** in six new files, 13 files, `+4842/−321`, **nothing under `src/domains/`, `src/services/`, `src/api/` or `src/context/`**, no dependency; **I1 closed at the level it was defined** (the one honest badge is read from the chat repository and disappears at zero and on an incomplete read; the attendance badge was removed rather than estimated); limitations recorded, browser QA NOT VERIFIED; its documentation reconciliation follows as its own commit |
| Product-feature phase | — | — | M3, M4, M5, M6, M7, M8, M9, M10, M11 and **Arena Frontend Freeze M-1..M-6 D7 D9** ✅ **COMPLETE** at `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb` on `arena/01a0b6be-parsian-music-dashboard-opus` (2026-09-19, docs-only). **M-1 Quick Actions CLOSED, D7 Accessibility CLOSED, M-2 Teacher Photo CLOSED, M-3 Student Photo CLOSED, M-4 I15 Failure Disclosure CLOSED, M-5 I16 Deep Links CLOSED, M-6 Settings Honesty CLOSED, D9 Performance CLOSED**. Known 10 `projectState.test.ts` failures preserved. **Browser QA NOT VERIFIED** as formal final-audit disposition. **Deferred do NOT modify frozen frontend:** Classes deep-link beyond capped list, Finance/Reports backend/domain, notification server wiring, localization/settings server wiring, working-hours/session-rules server wiring, free-slot search, external browser QA. **Backend boundary:** frontend freeze precedes Laravel/backend. **Canonical policy:** future fixes require explicit reopen decision. **HISTORICAL FREEZE PRESERVED — intentionally reopened below for FRONTEND PRODUCT COMPLETION.** **Next phase after the consolidation: ❌ NOT STARTED and NOT AUTHORIZED** — the Git-consolidation gate is discharged (PR #5 merged, `main` @ `f63ebaab37096fcb25063aca51c713e26d3b3da7`) but the ledger specifies no milestone beyond M11, **Laravel/backend is NOT STARTED**, and backend planning or implementation may begin **only after this documentation pass (D1/D2) is complete and the owner authorizes it explicitly**. |
| Frontend Product Completion — F0 Documentation/state closure | `e57bf1922e4ca3b80796715028445cfe8a077c9c` (PLANNING_CHECKPOINT) previous planning checkpoint `c244fec142fcc35faff8e8548c8ebbc05cb53bf5` preserved as historical, previous correction commit `a251e6942b2a645987286de6e822607554e84df9` documentation-only NOT planning milestone | ✅ yes (pushed) branch `arena/frontend-completion-spec` PR #4 CURRENT_BRANCH arena/frontend-completion-spec CURRENT_HEAD a251e6942b2a645987286de6e822607554e84df9 PLANNING_CHECKPOINT e57bf1922e4ca3b80796715028445cfe8a077c9c PREVIOUS_PLANNING_CHECKPOINT c244fec142fcc35faff8e8548c8ebbc05cb53bf5 | ✅ **COMPLETE — F0 Documentation/state closure** — CURRENT_BRANCH `arena/frontend-completion-spec` CURRENT_HEAD `a251e6942b2a645987286de6e822607554e84df9` = `a251e69` (actual Git HEAD before this F1-KICKOFF correction, frozen `a3867a6` descendant, previous planning checkpoint `c244fec` historical, previous correction commit `9695097` documentation-only NOT planning milestone) PLANNING_CHECKPOINT `e57bf1922e4ca3b80796715028445cfe8a077c9c` = `e57bf19` (last completed planning checkpoint) PREVIOUS_PLANNING_CHECKPOINT `c244fec142fcc35faff8e8548c8ebbc05cb53bf5` = `c244fec` preserved as historical — SHA semantics no longer ambiguous: CURRENT_HEAD = actual Git HEAD, PLANNING_CHECKPOINT = last planning checkpoint, PREVIOUS = historical, PREVIOUS_CORRECTION_COMMITS = documentation-only — do NOT call e57bf19 the current Git HEAD — spec path `docs/frontend-completion/` v14 @ `e57bf19` planning checkpoint + corrections (01-audit-reconstruction, 02-capability-matrix, 03-domain-map, 04-learning-access-policy, 05-rbac-access-control, 06-library-gallery-spec, 07-settings-theme-architecture, 08-export-architecture, 09-dashboard-analytics, 10-integration-architecture, 11-roadmap, 12-decision-register, 13-open-decisions, 14-handoff-checkpoint + README), PR #4 https://github.com/vahidateur/parsian-music-dashboard-opus/pull/4 docs-only, base `arena/01a0b6be-parsian-music-dashboard-opus` HEAD `02b7499`. **Current phase:** FRONTEND_COMPLETION — F0 COMPLETE + REVIEW CORRECTIONS + FINAL CHECKPOINT CORRECTION + SHA SEMANTICS CORRECTION, next F1. **Completed planning checkpoint:** F0 docs/state closure DONE + REVIEW CORRECTIONS docs-only pass DONE — PLANNING_CHECKPOINT e57bf19 exactly, PREVIOUS_PLANNING_CHECKPOINT c244fec historical only, CURRENT_HEAD a251e69 actual Git HEAD before this F1-KICKOFF correction, PREVIOUS_CORRECTION_COMMITS 9695097 documentation-only NOT planning milestone — 14 docs + corrections + SHA semantics correction — classification A/B/C evidence-based capability matrix domain map learning policy RBAC testable contract 11 cases with T-02 STATUS OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION library/gallery settings/theme export dashboard integration REQUIRED roadmap decision register open decisions handoff — SHA semantics no longer ambiguous. **Accepted decisions:** D1..D20, B1, NEW-LRN-01/LIB-01/GAL-01/SET-01/EXP-01/DASH-01/RBAC-01/PORTAL-01/CHAT-01/TG-01/BALE-01/MOB-01/CLASS-01 + 3 proposals PROP-BRAND-01 draft preview, PROP-LIB-01 level string vs relation, PROP-THEME-01 viewer vs org accent naming — do NOT change product decisions. **Open decisions:** 20 O-01..O-20 high-cost 7 O-01,O-02,O-08,O-09,O-10,O-13,O-14 before Laravel schema/API — T-02 remains explicitly STATUS OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION Teacher→unassigned student DENIED + ALLOWED evidences preserved — empirical to F2 must be resolved before RBAC implementation reaches this scope — do NOT change. **Blockers:** high-cost opens before backend, Telegram/Bale/Backup/Mobile integration contracts B require backend/integration layer but are REQUIRED PRODUCT CAPABILITY not optional, Finance/Reports I2/D6 C deferred, per_page 200 I16 mitigation disclosure, I13 view-boundary not closure 3 readers, I15 error discarding 14 consumers, L4 browser QA NOT VERIFIED. **Exact next slice:** **F1 — Learning + Level Access + Library + Gallery** — exactly ONE NEXT ACTION — canonical rule Level N=>1..N owner learning/eligibility.ts scope OPEN global vs per-program vs per-instrument, library filtering search/filter/sort/preview/locked/metadata, gallery genuine vs shallow + upload via media seam, absorb minor hardening Classes deep-link get(id) authoritative, pagination per_page mitigation, media one-frame issue into F1/F2. **Backend intentionally deferred:** Laravel/backend/migrations/controllers/routes/models, Finance/Reports domain money precision idempotency gateway, notification/localization/working-hours/session-rules server wiring, free-slot search, Telegram/Bale/Mobile integration implementation (contracts now, implementation later — but REQUIRED PRODUCT CAPABILITY, not optional), media storage S3/MinIO signed URLs scanning, export server streaming, dashboard server aggregation, RBAC backend enforcement org_id+permission+scope+per-object auth, student portal auth identity linking per-user cursor, backup encryption retention integrity restore. **Classification correction:** Telegram backup = REQUIRED PRODUCT CAPABILITY, Telegram student access = REQUIRED, Bale student access = REQUIRED, Mobile student client = REQUIRED — IMPLEMENTATION may remain deferred until backend/integration layer exists, adapter architecture Core domain/business logic → integration adapter → Telegram/Bale/Mobile, no business logic in bots. **Roadmap canonical sequence:** F0 Documentation/state closure COMPLETE, F1 Learning+Level Access+Library+Gallery NEXT, F2 RBAC+Ownership/Scope+Error Disclosure, F3 Settings+Theme Editor, F4 Export Engine, F5 Dashboard Analytics+Export, F6 Chat+Tickets+Files, F7 Student Portal, F8 Telegram+Bale+Backup integration contracts, F9 Mobile client contract, F10 Cross-surface QA+final frontend freeze. **Validation:** no src/tests/package/Laravel/backend/migration/protected branch/history rewrite, decision IDs unique, canonical engineering docs point same active branch CURRENT_BRANCH arena/frontend-completion-spec CURRENT_HEAD a251e6942b2a645987286de6e822607554e84df9 PLANNING_CHECKPOINT e57bf1922e4ca3b80796715028445cfe8a077c9c PREVIOUS_PLANNING_CHECKPOINT c244fec142fcc35faff8e8548c8ebbc05cb53bf5 spec @ e57bf19 planning checkpoint + corrections PR #4, SESSION_HANDOFF machine-readable CONTINUE HERE section CURRENT_BRANCH CURRENT_HEAD PLANNING_CHECKPOINT PREVIOUS_PLANNING_CHECKPOINT NEXT_ACTION F1 — SHA semantics no longer ambiguous. **FINAL CHECKPOINT CORRECTION — F1-KICKOFF CORRECTION — CURRENT_BRANCH arena/frontend-completion-spec CURRENT_HEAD a251e6942b2a645987286de6e822607554e84df9 PLANNING_CHECKPOINT e57bf1922e4ca3b80796715028445cfe8a077c9c PREVIOUS_PLANNING_CHECKPOINT c244fec142fcc35faff8e8548c8ebbc05cb53bf5 — previous correction commit 9695097 documentation-only NOT planning milestone — RBAC T-02 STATUS OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION — DENIED + ALLOWED evidences preserved — empirical to F2 — NEXT ACTION exactly ONE F1 — SHA semantics no longer ambiguous.** |
| **Consolidation — F0–F10 + governance merged into `main`** | `f63ebaab37096fcb25063aca51c713e26d3b3da7` (the PR #5 merge commit, and `main`'s tip) | ✅ yes (pushed — this **is** `main`) | ✅ **COMPLETE — PR #5 *Consolidate frontend F0–F10 + governance into main* MERGED 2026-09-20T21:11:18Z.** Merge parents: `a6469759fea743b9332702bb9a20e69fc4abc1be` (the previous `main` tip, **preserved as an ancestor — not rewritten**) and `112f1160104d375481b930517d08592bb67a0a72` (the PR #5 head). **PR #4 *docs: frontend product completion architecture + spec + execution map* is CLOSED and was NEVER MERGED**; its line `f00f85e6f817369a32b8921ffb64f58e17ea154e` reached `main` through PR #5 instead. **F0–F10 frontend product completion is DELIVERED** (`docs/frontend-completion/` 01–17 + README, plus the `F*_INVENTORY` / `F*_REPORT` engineering set on this branch), together with the Arena Frontend Freeze work (M-1..M-6, D7, D9), M10 `e7a64b7` on `98af31b` and the M11 chain `3ed0bfe→3eed4f1→24998d8→53794d5→f7eb867`. **The pre-Laravel Git-consolidation gate recorded in [PRE_CLEANUP_HANDOFF.md](PRE_CLEANUP_HANDOFF.md) §9–§11 is therefore DISCHARGED** — but **Laravel/backend is ❌ NOT STARTED and NOT AUTHORIZED**, and this row confers no new authorization. **No history was rewritten:** `main` advanced by merge only. Detail below → "Consolidation — 2026-09-20". |
...
## Product-feature phase — SPECIFIED (M0); M1, M2, M2.1, M3, M4, M5, M6, M7, M8, M9, M10 and M11 LANDED; no beyond-M11 milestone is specified, so there is no started next phase
...
`4e03b87`, reconciled at `e6ab6f9` + `8131c5c` — see "Product phase — M6" below) · **M7 ✅ complete**
(CP1 `0d9fc01`, CP2 `5726435`, CP3 `ede3ad6`, CP4 `f1ec0dd`, reconciled at `e7a6d72` —
...
**M8 ✅ complete** (one implementation checkpoint `735617d0324a8f4ba2e243846eedf069d389751a`,
**explicitly authorized by the owner on 2026-09-16**, built on the pre-M8 boundary
`a039ab0e2d1f0d339b91b23c6dffc3b0a4cc5046` — see the milestone table below and "M8's chain" under
"Documentation checkpoints") ·
**M9 ✅ complete** (one implementation checkpoint
`8d34eb3d1cd639cffc794596250c897b5ed4b6b3`, **explicitly authorized by the owner on 2026-09-16** and
built on `c186baaa1dda39e274369ed0387094822b00f0ae` — M8's documentation closure, the twenty-seventh
documentation checkpoint, which is therefore M9's **effective safe rollback boundary** — see the
milestone table below and "M9's chain" under "Documentation checkpoints") ·
**M10 ✅ complete** (one implementation checkpoint `e7a64b7777be36ddd97fc3337898fad794118e5b`,
**explicitly authorized by the owner** after the D5 finalization and built on
`98af31b5b04022362f0bc95ca99c426f2d7c8446` — the D5 finalization record, the twenty-ninth documentation
checkpoint by this ledger's count, which is therefore M10's **effective safe rollback boundary** — see the milestone table below
and "M10's chain" under "Documentation checkpoints") ·
**M11 ✅ complete** (four implementation checkpoints ending at `f7eb86762dc11f7d5dd3c7218e7c697fad29488e`,
**explicitly authorized by the owner** after the D7/D8/D9 decision record, starting SHA `3ed0bfe4ac928451a4bddf9de3a34f7be681c33f`
— the thirty-first documentation checkpoint by this ledger's count, which is therefore M11's
**effective safe rollback boundary** — see the milestone table below and "M11's chain" under
"Documentation checkpoints") · the phase specification names no milestone beyond M11, so there is
no started next phase and any further milestone would need a new **explicit owner authorization**.
...
| M7 — relation de-fixturing | fixture relations in the profile views + **I1** badges | ✅ **complete** — CP1 `0d9fc01079039548cf0ffe3c80c5bf2e88056a9c` (relation plumbing), CP2 `5726435506bd74748f6165819f3c80e30f359b80` (student relations), CP3 `ede3ad6a5e26b1cdfc514962b57ac1302309968c` (teacher relations) and CP4 `f1ec0ddde783aec14d6429ac2457f085f851ad9a` (class relations and navigation counts), reconciled at `e7a6d72e561bdbb0d22280e4617141281b62d8b5`; **67 new tests in six new files**, `git diff --name-only 8131c5c..f1ec0dd` outside `src/views/`, src/data/academy.ts (the design-time fixture module, dissolved at M10) and `src/components/layout/Sidebar.tsx` prints nothing; both full-suite failures recorded as environmental; **I1 closed**, browser QA NOT VERIFIED |
| M8 — branding application | the identity that is saved becomes the identity rendered (**D2** ✅ recorded 2026-09-16) | ✅ **COMPLETE** — one implementation checkpoint `735617d0324a8f4ba2e243846eedf069d389751a` (*feat(m8): render the persisted branding in the shell, the login screen and the tokens* — 6 files, 360 insertions / 26 deletions, one new 277-line test file with 15 cases), **explicitly authorized by the owner on 2026-09-16** and built on `a039ab0e2d1f0d339b91b23c6dffc3b0a4cc5046`, the pre-M8 boundary and therefore M8's **effective safe rollback boundary**; this pass registers it and cannot name itself; **browser QA NOT VERIFIED** |
| M9 — dashboard insight from live data | **H4**, with **I9** guards landing first | ✅ **COMPLETE** — one implementation checkpoint `8d34eb3d1cd639cffc794596250c897b5ed4b6b3` (*feat(m9): derive every dashboard insight from stored records, guarding the empty series first* — 14 files, 3 025 insertions / 328 deletions, of which four new test files carry **61 cases**), **explicitly authorized by the owner on 2026-09-16** and built on `c186baaa1dda39e274369ed0387094822b00f0ae` (M8's documentation closure, the twenty-seventh documentation checkpoint, and therefore M9's **effective safe rollback boundary**); **I9 landed first** and **H4** followed it, the four panels and the dashboard read no fixture value any more, the revenue chart was **removed** because no Finance/Reports seam exists (**D6**/**I2**, still planned), and **browser QA NOT VERIFIED** |
| M10 — fixture / type / seed separation | the separation shape recorded as the seven **D5** principles, the same-day D5 follow-ups (**F1** DEMO seed, **F2** legacy sessions/attendance, **F3** Hero, **F4** Finance/Reports) and the **D5 finalization record** (four-category model A/B/C/D and seventeen owner clauses), the documentation-drift table, **L2** | ✅ **COMPLETE** — one implementation checkpoint `e7a64b7777be36ddd97fc3337898fad794118e5b` (*fix(reliability): M10 — dissolve the fixture boundary, no architectural sociology* — 82 files, 2 122 insertions / 1 942 deletions, incl. one new gate `src/__tests__/m10Boundary.test.ts` with 13 checks, twelve new owner modules, sixteen fixture-era gates mechanically re-derived), **explicitly authorized by the owner** after the D5 finalization and built on `98af31b5b04022362f0bc95ca99c426f2d7c8446` — the D5 finalization record, the twenty-ninth documentation checkpoint by this ledger's count, which is therefore M10's **effective safe rollback boundary**; D5's four categories landed with **one owner each** — types with their domains, the canonical DEMO seed at `src/domains/demo/academySeed.ts` (data unchanged), fabricated UI measurements **removed** and classified one by one in `src/lib/financeReportsDeferral.ts` (**D6**/**I2**), presentation vocabulary/config in the seventeen named owners the gate enforces; `freeSlotsTuesday` removed from executable production data while `nlCommands` n3 answers **honest typed no-data** — **no free-slot backend built**; Hero identity via the M8 branding seam and `useAuth` — **no `BrandingSettings` persistence/schema change** — with the fabricated status/peak claims removed; **F2/I8 untouched** (no schema, IDs, collection semantics or backup-envelope change — zero diff in those domains); Finance/Reports stay **deferred** (visible, testable deferral; **I2 remains open**); **L2 stays open** (its owner file relocated with the code to `src/domains/teachers/types.ts`); **docs errata:** the implementation report's `ClassSession.resourceId` removal claim was false (the field never existed — type relocation only) and its demoStore R4/R6 claim described **pre-existing** lifecycle machinery (`src/services/demoStore.ts` changed by one type-import re-point only); **branch-name errata:** the designated arena/01a0a9b6 name does not exist — the actual implementation branch is arena/01a0aa83-parsian-music-dashboard-opus, unaltered; **browser QA NOT VERIFIED** |
| M11 — performance, api disclosure, a11y, browser QA | **I6**, **D7**, **D8**, **D9**, **L4** | ✅ **COMPLETE** — four implementation checkpoints built on the D7/D8/D9 decision record `3ed0bfe4ac928451a4bddf9de3a34f7be681c33f`, which is therefore M11's **effective safe rollback boundary** (starting SHA = `3ed0bfe`; final implementation HEAD = `f7eb867` — an implementation-report wording that labelled `f7eb867` the starting SHA is corrected here): `3eed4f190b6538ad62bf35d7b0c9fa5afc310acc` (perf — lazy academic/operations workspace chunks, React-dependent vendor chunk, `chunkSizeWarningLimit` deliberately unraised, dead Vazirmatn **300/800** imports removed), `24998d8330a95510cd969c872edb28ddcc59ef16` (test — the D9 budget gate encoded from the M11 measurements), `53794d57a7375c8345f76adec9248f5da09d7a92` (feat — **D8**: the single `DEMO_SERVED_DOMAINS` enumeration and the persistent api-mode-only, health-claim-free notice, six tests) and `f7eb86762dc11f7d5dd3c7218e7c697fad29488e` (test — **D7**: the dependency-free accessibility gate, 14 cases, stable across three runs). **Final D9 measurements:** entry JS gzip **108 207** B, vendor gzip **60 108** B, total JS+CSS+HTML gzip **298 249** B, largest chunk **108 207** B, `dist/` raw **1 964 789** B; the >500 kB raw warning is cleared by real splitting. Protected areas untouched (I2, L7, backend/CSP/persistence/Compensation/Attendance/M10 boundary), no dependency change; limitations recorded (PROJECT_STATE.md §4 → "M11 validation", §7 item 23), **browser QA NOT VERIFIED** (L4 — external-QA disposition preserved); its documentation closure follows as its own commit |
| Arena Frontend Freeze — M-1..M-6, D7, D9 CLOSED | **M-1 Quick Actions, D7 Accessibility, M-2 Teacher Photo, M-3 Student Photo, M-4 I15 Failure Disclosure, M-5 I16 Deep Links, M-6 Settings Honesty, D9 Performance** | ✅ **COMPLETE — frozen at `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb` on canonical `arena/01a0b6be-parsian-music-dashboard-opus` (2026-09-19, documentation-only).** M-1 real dialogs no hardcoded arrays, D7 focus-trap/RTL/reduced-motion, M-2/M-3 photoMediaId via Media/ProfilePhotoField, M-4 ErrorState+retry not empty/zero, M-5 `get(id)` authoritative beyond 200, M-6 Toggle disabled + explicit deferral Surfaces, D9 entry 118.5k gzip <273791 total ~300k within 305805 cap no chunk ≥400k 2037 modules. Typecheck 0 errors, focused 92 pass, full suite 2102 passed / 10 failed — 10 failures known historical `projectState.test.ts` SHA/branch expectations not regressions test not altered. **Browser QA NOT VERIFIED** as formal final-audit disposition no claim complete, no claim backend exists. **Deferred do NOT modify frozen frontend:** Classes deep-link beyond capped list (`src/views/Classes.tsx:678`), Finance/Reports backend/domain, notification server wiring, localization/settings server wiring, working-hours/session-rules server wiring, free-slot search, external browser QA. **Backend boundary:** frontend freeze precedes Laravel/backend. **Canonical policy:** future fixes require explicit reopen decision. Preserved history: M11 chain `3ed0bfe→3eed4f1→24998d8→53794d5→f7eb867`, M10 `e7a64b7` on `98af31b`, working branch previously `arena/01a0aa83` at `d31cbe9`, other branches unchanged. |
...
room and time, the finance panel's «۱۲ روز از سررسید» — was **H4/M4/M9** work and was deliberately
left alone: M2 removed false claims about writes, it did not make these surfaces truthful overall.
*(Correction, 2026-09-16: **M4** removed the scheduling half and **M9** removed the dashboard panels'
half at `8d34eb3d1cd639cffc794596250c897b5ed4b6b3`; the **Finance** figure named here is still on
screen and stays with **I2**/**D6**.)*
...
and `TODAY_INDEX` from src/data/records.ts (the design-time fixture module, dissolved at M10): a frozen weekday as "today", a hardcoded room-4 transfer
...
fixture collections in src/data/records.ts (the design-time fixture module, dissolved at M10) (`src/views/Classes.tsx` and `src/views/Attendance.tsx`
...
`attendanceTrend` and `attendanceByDay` from src/data/records.ts (the design-time fixture module, dissolved at M10); it printed a hardcoded «نرخ حضور
...
a different view, still fixture-driven, and **I2**'s (deferred by **D6**) — *(correction, 2026-09-16:
this line attributed it to "**I2** and M9's"; **M9** landed at `8d34eb3d1cd639cffc794596250c897b5ed4b6b3`
without touching this view or creating a domain, so the attribution is corrected here rather than left
...
registers "nine" where src/data/records.ts (the design-time fixture module, dissolved at M10) holds eight, and dates the domain "since M1" where it
...
## Product phase — M7 — relation de-fixturing + sidebar badges (**I1**) — ✅ COMPLETE

**Documentation checkpoint (CP0):** **none — and that is recorded, not hidden.** M7's scope was already
accurate in the section M6's reconciliation left behind
([PRODUCT_PHASE_SPECIFICATION.md](PRODUCT_PHASE_SPECIFICATION.md) → M7), so the milestone went straight
to implementation and the reconciliation happened **after** its evidence existed — the same accepted
ordering M5 and M6 used. CP1 is a product commit that also creates the milestone's own gate; it is not a
documentation checkpoint, because it changes tests and source, not documents.

**Implementation checkpoints (four) · all pushed ✅**
- **CP1** `0d9fc01079039548cf0ffe3c80c5bf2e88056a9c` · *feat(m7): add relation surface plumbing* ·
  4 files, **1 073 insertions, 0 deletions** — no product source, no view: the per-surface fixture gate
  (`src/views/__tests__/relationsNoFixtures.test.ts`, five surfaces — plumbing, students, teachers,
  classes, navigation — each `staged` or `enforced`, every rule carrying a probe so a rule that protects
  nothing fails), the through-route plumbing suite (`relationsPlumbing.test.ts`), and the two shared
  helpers the surfaces need: `src/views/relations/academyDay.ts` (the academy's calendar day and its
  Saturday-first weekday index, reformatted from `academyNow()` — no `new Date()`, no second clock) and
  `src/views/relations/indexById.ts` (one id-index per loaded page, so a foreign key is not resolved by
  a per-row scan and the fixture resolvers have no reason to return).
- **CP2** `5726435506bd74748f6165819f3c80e30f359b80` · *feat(m7): de-fixture student relations* ·
  3 files, **899/85** — the student profile's class, teacher, room and weekly-session relations now come
  from `useClasses`, `useTeachers`, `useRooms`, `useEnrollments` and a bounded
  `useSessions({ from, to, per_page })` window anchored on the academy day. **7 new tests.**
- **CP3** `ede3ad6a5e26b1cdfc514962b57ac1302309968c` · *feat(m7): de-fixture teacher relations* ·
  3 files, **1 330/134** — a teacher's classes, students, rooms and week come from the teachers' own
  domain reads; `TODAY_INDEX`, `classById`, `studentById` and `weekSessions` leave the view; and the
  detail resolves its selection by id with the consumer-side key guard (`key={detail.id}`) because
  `useStudentList` is the one list hook without the render-time query-identity invariant (**I13**).
  **11 new tests.**
- **CP4** `f1ec0ddde783aec14d6429ac2457f085f851ad9a` · *feat(m7): de-fixture classes and navigation* ·
  6 files, **1 606/168** — instructor and studio through `useTeachers`/`useRooms`; the roster from the
  **canonical enrollment relation** (active enrollments joined to the student rows) and never
  `AcademyClass.studentIds`; seat, waitlist and occupancy figures from that same bounded read, printing
  «—» with an «N ردیف از M» note on a partial page; the week as one bounded windowed session read; the
  rooms panel from the room repository with an explicit empty state; the fabricated deltas
  (`delta: 3.4` / `delta: 2.1`) and the `TODAY_INDEX`/`weekSessions` coupling gone. In the chrome:
  `NavDef.badge` **removed** from src/data/academy.ts (the design-time fixture module, dissolved at M10); the messages badge summed from the conversation
  rows the chat repository returns, rendered only while that read is complete and positive; the
  attendance badge and its «۳ کلاس ثبت‌نشده» hint **removed** because no scoped, pageable
  unrecorded-sessions read exists; the command hints reworded to name a target or a filter; the
  quick-action room option and recipient group stripped of their figures. **20 new tests** (13 classes,
  7 navigation).

**Size (from `git diff --shortstat 8131c5c..f1ec0dd`):** 13 files, 4 842 insertions, 321 deletions — the
three views, src/data/academy.ts (the design-time fixture module, dissolved at M10), `src/components/layout/Sidebar.tsx`, two new modules under
`src/views/relations/` and six new test files. **No file under `src/domains/`, `src/services/`,
`src/api/` or `src/context/` changed at all** — no contract, no repository verb, no seed, no fixture, no
type — and no dependency was added.
**Built on:** `8131c5cd49c35a7163543082475a9635914aba40` — **M6's coverage matrix**, which is therefore
M7's **effective safe rollback boundary**: rolling back to it drops M7 entirely and keeps M6 and every
document it left behind.

**Validation.** One full-suite run at `f1ec0ddde783aec14d6429ac2457f085f851ad9a`, `dist/` present:
**125 files / 1 729 passed / 0 skipped / 2 failed**, duration 189.90 s — the suite moved from M6's
recorded 119 files / 1 655 passed / 8 skipped / 1 failed by **+6 files** and **+67 cases**, the eight CSP
cases that stop skipping once a build exists, and one new environmental failure (below). Typecheck clean,
`npm run build` exit 0, `git diff --check` clean, and **I16**'s ceilings, **I13** and the protected groups
unmoved: `git diff --name-only 8131c5c..f1ec0dd -- src/domains/` prints nothing. **Both failures are
environmental and recorded, never silenced:** the known branch-name case
(`projectState.test.ts:299`, [PROJECT_STATE.md](PROJECT_STATE.md) §7 item 16) and a **date-dependent**
case in `src/views/__tests__/schedulingWrites.test.tsx` that appeared when the sandbox clock reached
Tuesday 2026-09-15 — the weekday the fixture assigns to `cl2` — and that reproduces identically on the
**pre-CP4 tree**, in a file, domain and seed M7 never touched (§7 item 18 and
[OPEN_ITEMS.md](OPEN_ITEMS.md) I19). **The new tests are pinned by reversion evidence:** against the
pre-CP4 implementations — restored sha256-verified and then restored back byte-exactly — **17 of the 20
CP4 cases fail**, and seven gate injections (a `studentIds` roster, a fixture re-import, a removed
`key={detail.id}`, a static attendance badge, a digit in a hint, a room-availability percentage, a
parenthesised option count) each turn the gate red; the last one exposed a real gap, so the gate gained
an `options: [ … (N) … ]` rule in both digit scripts and the injection was re-proved red. Full detail is
[PROJECT_STATE.md](PROJECT_STATE.md) §4 → "M7 validation" and its requirement-by-requirement
**coverage matrix**.

**What M7 does *not* claim.** The fixtures are **not deleted** — M7 removed *readers*, not data, and the
three roles (entity types, the canonical DEMO seed, fake data for unwired views) are untouched
(**M10/D5**). `src/views/Messages.tsx` (`messageTemplates`), `src/views/Library.tsx` (`libraryShelves`)
and `src/views/Settings.tsx` (`settingsSections`) still import fixture content, and the dashboard insight
panels are still fixture renderers (**M9/H4** — *(correction, 2026-09-16: that half was closed by **M9**
at `8d34eb3d1cd639cffc794596250c897b5ed4b6b3`; the three views above and the design-system gallery's
fixture samples remain, with **M10/D5**)*): the milestone's original scope named those views but the
authorized checkpoints did not include them, and the product-wide boundary test the final-phase DoD asks
for does not exist yet. **`useStudentList` is still un-keyed (I13)**: M7 reached it from three surfaces
and mitigated the exposure with the consumer-side key guard the gate now requires — a mitigation, not a
closure. **The `per_page: 200` ceilings stay (I16)**, stated and disclosed per surface rather than
removed. **The command palette's substring matching is untouched (L1)** — the false numbers went, the
loose matching stayed. **No acceptance audit ran**, so M7 has no audit findings and no **B**-numbered
coverage items; the reversion and mutation evidence above is the implementer's own measurement. And
**browser QA has never run and is NOT VERIFIED** ([PROJECT_STATE.md](PROJECT_STATE.md) §5) — the three
rewired surfaces and the navigation badge are jsdom-verified only.

**Rollback boundaries.** **Spec:** `9190da02a8ddcc49f7fe1ae010e5a3a9b79c48b9` (the boundary M6's section
recorded for M7, written before M6's reconciliation existed). **Effective safe:**
`8131c5cd49c35a7163543082475a9635914aba40` — the commit M7 was actually built on, which keeps M6 and
every document intact; rolling back past it would drop M6's coverage matrix as well. No history was
rewritten, reset or rebased to make these documents agree.

---

## Workstream — Class Compensation P1 (make-up for a cancelled one-to-one session) — ✅ SHIPPED at `a21311d`

**Not a milestone, and the M0–M11 sequence is untouched.** P1 is work authorized outside the
specification's milestone order, like the I13 checkpoints before it: it closes no **M** milestone, it
occupies no row of the milestone table, and it advances no phase-checkpoint row (that row stays at
Phase 2 for the ordering reason in [PROJECT_STATE.md](PROJECT_STATE.md) §2). It is registered here
because a commit this ledger cannot explain is exactly what this document exists to prevent.

**Base and rollback boundary.** P1 was built on M7's documentation reconciliation
`e7a6d72e561bdbb0d22280e4617141281b62d8b5`, which is therefore its **effective safe rollback
boundary**: rolling back to it drops P1 entirely and keeps M7 and every document it left behind. The
workstream commit itself is `a21311d7e32c82e3a46b1581c94f6b3478bf646c`.

**Implementation checkpoint (one) · pushed ✅**
- `a21311d7e32c82e3a46b1581c94f6b3478bf646c` · *feat(compensation): add the make-up obligation domain*
  · **18 files, 2 976 insertions, 0 deletions** — the new `src/domains/compensation/` (types, the pure
  derivation, the five-verb contract, the demo implementation, the read layer, a README and **69 tests
  in five files**) plus seven existing files, **every one of them insertions only**: the dataset
  collection (`src/domains/demo/types.ts`, `src/domains/demo/seed.ts`, `src/services/demoStore.ts`), the
  reference-integrity rules (`src/domains/demo/backup.ts`), the registry seam
  (`src/domains/registry.ts`), and the two **additive integration edits the dataset contract required**
  (`src/components/settings/DemoDataPanel.tsx`, `src/domains/demo/__tests__/seed.test.ts`). **Nothing
  under `src/domains/scheduling/` or `src/domains/attendance/` changed**, `Session` and `SessionStatus`
  are untouched, and no dependency was added.
...
The make-up is an ordinary session created through the scheduling repository's own `create()` verb, and
the one-hour same-day shape is a **prefill** of the original's own date, room and teacher — never an
automatic booking and never a free-slot search.

**What it does *not* claim.** **No UI and no `apiRepository`** — nothing in the product offers the flow,
and api mode would silently serve the demo implementation for an eleventh domain (**D8**; catalogued as
**I20**); **no notification of any kind** (**D1**, **I7**); **group and class-wide compensation are
refused by design** (eligibility is `kind === "private"`, never roster size), so the requirement
recorded as **I18** is only **partly landed** and **I18 stays OPEN**; the demo dataset ships **no
compensable case**; there were **no mutation and no reversion checks for P1's own commit** — the C-1,
C-1.1 and C-2 packages that followed each carried their own, ten in total, every one restored
byte-identically against a recorded sha256 baseline (see the chain paragraph below); **automatic
completion of elapsed sessions is a separate workstream — not implemented, not authorized and
deliberately kept out of P1**; and **browser QA has never run** on any of it, because there is no
surface to open. Full record: [PROJECT_STATE.md](PROJECT_STATE.md) §3 → "Last completed work (Class
Compensation P1…)", §4 → "Compensation P1 validation" and §7 item 19; decisions **D18**/**D19** in
[DECISIONS.md](DECISIONS.md).

**Validation** (measured at `a21311d`, `dist/` present): `npx tsc --noEmit` clean, `npm run build` exit
0, `git diff --check` clean, and the full suite at **130 files / 1 800 tests → 1 798 passed / 0 skipped /
2 failed** — both failures the recorded environmental pair (the branch-name case and **I19**), unchanged
by P1 — with Group A at 211 and Group D at 79 untouched and the 69 new cases green.

**The workstream continued past P1 with the secretary surface (2026-09-15/16) — three product commits,
all pushed, none of them a milestone.** `fc83d6d` (*feat(compensation): add the secretary compensation
surface* — 7 files, 2 243 insertions / 1 deletion) wires the read layer and the three write verbs into a
Persian RTL surface on the existing design system: the ledger read and its count reads go through
`useCompensations`, the domain's own eligibility, frozen-student, derived-state, live/terminal, lineage
and uniqueness rules are **consumed, never re-derived**, every action addresses the effective attempt's
`currentAttempt.sessionId` (never `bookedSessionId`), a teacher gets no write control, and the route is
registered through the RBAC permission that owns it (`schedule.read` to see the surface,
`schedule.write` for register / schedule / complete) in `src/App.tsx`, src/data/academy.ts (the design-time fixture module, dissolved at M10),
`src/components/layout/Sidebar.tsx` and `src/domains/auth/permissions.ts`. `79fd44e`
(*fix(compensation): harden UI count states* — 2 files, 197/28) made an in-flight or failed count render
the indeterminate `NO_DATA` («—») instead of a number, named the failure and let each read retry itself.
`79ec13d` (*fix(compensation): complete UI hardening* — 7 files, 478/81) closed the remaining findings
of a read-only audit of that surface, each with its own case in the focused suite:

- **S-1 — a failed ledger *refresh* is disclosed, not hidden.** The surface keeps the rows of its last
  successful read and says so («فهرست جبرانیها تازهسازی نشد») with a `ledger.reload()` retry; a
  failed *first* read is still an error, never an empty ledger.
- **S-2 — the warning-consent toggle says what it accepts.** Its Persian label names the warning kinds
  the scheduling engine actually raises instead of only the off-schedule one, with no duplicated
  conflict semantics and no new warning type.
- **S-3 — a failed effective-session read is a failure, not missing data.** The drawer names the failed
  read (not tooltip-only) and refuses the move until it resolves, while the genuinely-missing attempt
  stays the state it always was.
- **S-4 — the bounded candidate search is stated.** The register dialog discloses its window (365 days
  back / 120 days forward) and its 200-row page cap, and reports truncation and an honest empty result
  instead of implying completeness. No window was expanded and no free-slot search was added.
- **S-5 — the C-2 disclosure assertion was scoped to the drawer that owns it** (a body-wide assertion
  could be satisfied by a toast), and the `undefined` branch is documented rather than covered by an
  impossible-state test. **C-2's semantics are unchanged.**
- **S-7 — one Jalali date-input boundary.** `src/views/shared/jalaliInput.ts` (`DATE_INPUT_OPTIONS`,
  `jalaliInputValue`, `FIELD_MESSAGES`) is adopted by both scheduling dialogs and both compensation
  dialogs; the boundary sits in the view layer, never in the Scheduling domain, and behaviour is
  exact.
- **S-8 — the real route is covered.** `src/__tests__/routeProtection.test.tsx` gained the surface's
  own two cases: an administrator reaching `#/compensation` through the shell (no silent Dashboard
  fallback) and a role without `schedule.read` being refused, with no routing redesign.
- **S-9 — one submit/refusal wrapper.** The three dialog submits share one helper; refusal feedback,
  loading state, thrown-error handling and success behaviour are unchanged.
- **S-6 — deliberately left open.** The surface is pinned by its own focused suite and by the gates that
  scan every view, but it is **not** enumerated in the per-surface lists (`relationsNoFixtures`'
  `SURFACES`, `emptyEnvironment`'s view lists, `writeFeedbackHonesty`'s `GRADUATED_VIEWS`) and has no
  dedicated list-driven fixture suite. Closing that would change the gate architecture and test lists,
...
18, **I19**); `npm run build` exit 0 (4.19 s, the pre-existing chunk-size warning unchanged); `git diff
--check` clean; and the full suite at **1 868 passed / 2 failed of 1 870** — the two failures the
recorded environmental pair (the branch-name case at `projectState.test.ts:299` and **I19**'s
date-dependent case), neither of them a compensation regression. Groups A (211) and D (79) and the
domain's 121 cases are untouched: no commit in the chain lists a file under
`src/domains/scheduling/`, `src/domains/attendance/` or `src/domains/compensation/`.

---

## Consolidation — 2026-09-20 — F0–F10 + governance merged into `main` (PR #5)

**This is a ledger event, not a product milestone.** It changes no product behaviour, closes no
open item, and **authorizes nothing**. It records that the Git consolidation the previous session
was blocked on is **complete on the remote**, and exactly what it settled.

| Fact | Value |
|---|---|
| PR #5 | [pull/5](https://github.com/vahidateur/parsian-music-dashboard-opus/pull/5) — *Consolidate frontend F0–F10 + governance into main* — **MERGED** 2026-09-20T21:11:18Z |
| Merge commit = canonical tip | `f63ebaab37096fcb25063aca51c713e26d3b3da7` on `main` |
| Merge parents | `a6469759fea743b9332702bb9a20e69fc4abc1be` (previous `main` tip — **an ancestor, preserved**) and `112f1160104d375481b930517d08592bb67a0a72` (PR #5 head) |
| PR #4 | [pull/4](https://github.com/vahidateur/parsian-music-dashboard-opus/pull/4) — **CLOSED, NEVER MERGED**. Head `f00f85e6f817369a32b8921ffb64f58e17ea154e` on `arena/frontend-completion-spec`; base `arena/01a0b6be-parsian-music-dashboard-opus` |
| F0–F10 | ✅ **DELIVERED** on canonical history — `docs/frontend-completion/` 01–17 + README |
| Frontend freeze line | ✅ preserved — M-1..M-6, D7, D9 at `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb` |
| M10 / M11 chains | ✅ preserved — M10 `e7a64b7777be36ddd97fc3337898fad794118e5b` on `98af31b5b04022362f0bc95ca99c426f2d7c8446`; M11 `3ed0bfe4ac928451a4bddf9de3a34f7be681c33f` → `3eed4f19` → `24998d83` → `53794d57` → `f7eb86762dc11f7d5dd3c7218e7c697fad29488e` |
| Governance | ✅ on `main` — [GOVERNANCE_CHECKPOINT.md](GOVERNANCE_CHECKPOINT.md), reached via [DECISIONS.md](DECISIONS.md) §20 |
| History rewrite | ❌ **None.** `a6469759fea743b9332702bb9a20e69fc4abc1be…main` is *ahead 49 / behind 0*, so the old tip is an ancestor and the advance was merge-only — no rebase, no squash, no force-push |
| Four historical SHAs preserved | `f00f85e6f817369a32b8921ffb64f58e17ea154e` · `02b74996e6458d10a5d9e8d1023890239342c4e4` · `112f1160104d375481b930517d08592bb67a0a72` · `59fdacf9606fdc2ded3f060603405f94c7408d30` (in `main`'s own first-parent history: `…7a44c6a → 59fdacf9 → 112f1160 → f63ebaab`) |
| Pre-Laravel gate | ✅ **DISCHARGED** — [PRE_CLEANUP_HANDOFF.md](PRE_CLEANUP_HANDOFF.md) §10 step 14 ("Only then start Laravel/backend") is satisfied through step 13 |
| Laravel / backend | ❌ **NOT STARTED and NOT AUTHORIZED** |

**Next phase.** `main` is consolidated and the pre-Laravel Git gate is discharged — but that is a
**precondition, not a work order.** **Laravel/backend remains ❌ NOT STARTED and NOT AUTHORIZED.**
Backend planning or implementation may begin **only after the documentation gate (the D1/D2
documentation pass recorded in [PROJECT_STATE.md](PROJECT_STATE.md)) is complete**, and only on an
explicit owner authorization. Nothing in this ledger row authorizes a milestone beyond M11, any
F-series implementation, or any `src/` change.

**Historical phase entries above are unchanged by this record.** No M-row, milestone, coverage
matrix or validation block has been rewritten, and no past status has been re-stated as something
...
| `d18d3e39b513a9031ba8cc8383e7b98d7ae99c99` | **the Class Compensation UI's documentation reconciliation** (*docs(governance): reconcile compensation UI checkpoints*) | The secretary surface's chain registered (`fc83d6d` the surface, `79fd44e` and `79ec13d` its two hardening packages), the audit findings **S-1**…**S-9** recorded as closed with **S-6** left open as **I21**, the surface's validation block written into [PROJECT_STATE.md](PROJECT_STATE.md) §4, the test catalogue and the limitations brought up to it, and the decision and backlog records (D18–D20, **I18**, **I20**) reconciled with what had landed. **The twenty-third** documentation checkpoint: **documents only** (five `docs/engineering/` documents, 309 insertions / 79 deletions), no product source, no test, no dependency. It could not name itself — this pass names it — and it is the commit **the S-6 boundary gate was built on** | none (documents only) | ✅ |
| `9fcf00891d07db18c718296ccaddc79919c793c1` | **the I21 closure and the S-6 gate registration** (*docs(compensation): close I21 and register the S-6 boundary gate*) | The dedicated per-surface boundary gate registered as a **chain commit** (`bf8bd911`, one new 768-line test file, 40 cases), **I21** closed, and the six gate-architecture findings left deferred | none (documents only) | ✅ |
| `3acb5f3be275a67726376adbfed3f3fdf2f1282b` | **the D2 record** (*docs(governance): record decision D2 before M8*) | **D2** decided with its row, its four-field section and the register's counts (fourteen decided, four open) — the shipped default name «آموزشگاه موسیقی پارسیان» — and `9fcf008` registered as the twenty-fourth documentation checkpoint | none (documents only) | ✅ |
| `a039ab0e2d1f0d339b91b23c6dffc3b0a4cc5046` | **the M8 boundary reconciliation** (*docs(m8): reconcile the M8 base/rollback boundary and register checkpoint 25*) | §5's M8 checkpoint paragraph corrected so the boundary it names is the twenty-fourth documentation checkpoint `9fcf008` instead of M7's superseded reconciliation `e7a6d72` — whose M7 *product* record nevertheless stands — described consistently with §12's rollback rule and [PROJECT_STATE.md](PROJECT_STATE.md) §2's no-self-reference rule; `3acb5f3` registered as the twenty-fifth documentation checkpoint in the table and in the chain paragraph; and the M8 row in the milestone table left stating explicitly that M8 was **not authorized** at that point. It is the commit **M8 was built on** | none (documents only — three `docs/engineering/` documents, 32 insertions / 15 deletions; no product source, no test, no dependency, no build) | ✅ |
| `c186baaa1dda39e274369ed0387094822b00f0ae` | **M8's documentation closure** (*docs(m8): reconcile the branding phase record and register the milestone*) | M8 registered ✅ **COMPLETE** against its measured evidence — the single implementation checkpoint `735617d0324a8f4ba2e243846eedf069d389751a`, its six files and 15 cases, the §4 validation block, **D2** reconciled from *required* to *recorded* now that the milestone had landed, and the out-of-scope fixture residue (`src/components/hero/Hero.tsx`, `src/domains/demo/seed.ts`) kept recorded rather than smoothed over. **The twenty-seventh** documentation checkpoint, which could not name itself — the M9 closure pass names it — and **the commit M9 was built on** (four `docs/engineering/` documents; no product source, no test, no config, no gate) | none (documents only) | ✅ |
| `3ed0bfe4ac928451a4bddf9de3a34f7be681c33f` | **Latest recorded here.** **The D7/D8/D9 decision record** (*docs(m11): record D7/D8/D9 decisions and the browser-QA environment disposition*) | The three decisions gating M11 recorded ahead of any implementation authorization — D7's dependency-free accessibility strategy, D8's api-hybrid disclosure behaviour on the **eleven** demo-in-both-modes getters, D9's measurement-derived budget on the real `c16890f` baseline — plus §5's browser-QA environment disposition (**NOT VERIFIED** is this environment's only honest disposition); the record explicitly **did not authorize** M11. Documents only. **The thirty-first** documentation checkpoint *by this ledger's count*, which could not name itself — it is named by **this** pass, the M11 documentation closure. It is the commit **M11's implementation chain was built on**, which makes it M11's **effective safe rollback boundary** | none (documents only) | ✅ |
| `c16890f5814d6ff40cd95f59be2214d2f1bcf82e` | **M10's documentation closure** (*docs(m10): close the fixture-dissolution phase and register the M10 checkpoint*) | M10 registered ✅ **COMPLETE** against its measured evidence — the single implementation checkpoint `e7a64b7777be36ddd97fc3337898fad794118e5b`, the §4 validation block, the milestone table, the M10 chain, the D5 implementation-resolution note and the two implementation-report **errata** (the `ClassSession.resourceId` claim and the demoStore R4/R6 description) kept recorded rather than smoothed over; the stale dissolved-fixture references were reconciled and `docs/architecture/demo-data.md`'s contradiction was deliberately **not** fixed (scope — recorded as **L7**). Documents only — eight authorized files; no product source, no test, no config, no gate. **The thirtieth** documentation checkpoint *by this ledger's count*, which could not name itself — it is named by **this** pass, the D7/D8/D9 decision record. It is the commit the D7/D8/D9 record was built on | none (documents only) | ✅ |
| `98af31b5b04022362f0bc95ca99c426f2d7c8446` | **The D5 finalization record** (*docs(governance): finalize decision D5 — four-category model and seventeen owner clauses*) | The separation shape M10 executed was recorded and **FINALIZED**; the stale L3 scope references, the “third role already unused” premise and the fixture census were corrected; **L2** and the five-name drift table stayed scoped for M10. **The twenty-ninth** documentation checkpoint *by this ledger's count — the previously registered one is the twenty-eighth `3a16549` —* which could not name itself — it is named by **this** pass, the M10 closure. It is **the commit M10 was built on** (documents only; no product source, no test, no config, no gate) | none (documents only) | ✅ |
| `3a16549989f6ac41ba550e18107c62ec896f32a5` | **M9's documentation closure** (*docs(m9): close the dashboard insight phase and register the M9 checkpoint*) | M9 registered ✅ **COMPLETE** against its measured evidence — the single implementation checkpoint `8d34eb3d1cd639cffc794596250c897b5ed4b6b3`, the §4 validation block, the milestone table, the M9 chain and the limitations reconciled. **The twenty-eighth** documentation checkpoint, which could not name itself — it is named by **this** pass, the D5 record, which also corrects the stale L3 scope references, the stale "third role already unused" premise and the stale fixture line/import counts. It is the commit the D5 record is built on (documents only; no product source, no test, no config, no gate) | none (documents only) | ✅ |
| `f7eb86762dc11f7d5dd3c7218e7c697fad29488e` **is not a documentation checkpoint** — it is M11's final implementation HEAD (see M11 row). The **thirty-second** documentation checkpoint is the **M11 documentation closure** (the commit that registered M11 ✅ COMPLETE against its measured evidence — four checkpoints `3eed4f1→24998d8→53794d5→f7eb867` on `3ed0bfe`, final D9 values entry 108207 B vendor 60108 B total 298249 B, D8 eleven domains, D7 14 cases, L4 NOT VERIFIED, protected areas untouched, 7 `projectState` environmental failures, limitations §7 item 23). It could not name itself — this ledger names it here and the freeze closure names it as its boundary. | **M11 documentation closure** — M11 registered ✅ COMPLETE, 32nd documentation checkpoint | M11's chain preserved, no product source change, no dependency, no backend | ✅ |
| `a3867a60ac25c81adc5c1bc10122f4eeeedbc0cb` **is not a documentation checkpoint** — it is **Arena Frontend Freeze frozen product HEAD** (M-1..M-6 D7 D9 CLOSED). The **thirty-third** documentation checkpoint is **this pass** — *docs: freeze Arena frontend checkpoint* — the **Arena Frontend Freeze closure** that records frozen HEAD `a3867a6` on canonical `arena/01a0b6be-parsian-music-dashboard-opus` (2026-09-19, docs-only), M-1..M-6 D7 D9 CLOSED, 10 known `projectState.test.ts` failures preserved test not altered, **browser QA NOT VERIFIED** as formal final-audit disposition no claim complete no claim backend exists, deferred do NOT modify frozen frontend (Classes deep-link beyond capped list, Finance/Reports backend/domain, notification server wiring, localization/settings server wiring, working-hours/session-rules server wiring, free-slot search, external browser QA), backend boundary frontend precedes Laravel/backend, canonical policy future fixes require explicit reopen decision, historical checkpoints preserved (M11 chain `3ed0bfe→f7eb867`, M10 `e7a64b7` on `98af31b`, working branch previously `arena/01a0aa83` at `d31cbe9`, other branches unchanged). It cannot name itself — the next documentation commit must name it. | **Arena Frontend Freeze closure** — frozen at `a3867a6`, M-1..M-6 D7 D9 CLOSED, 33rd documentation checkpoint | no product source, no test, no dependency, no backend, no browser QA complete claim, only `docs/engineering/` | ✅ |
| `6ca25be654b56579054c88cd498a42f57ea63582` | **The D1 pass** (*docs: D1 — record the post-consolidation canonical state*) — the post-consolidation canonical state recorded on top of the PR #5 merge: `main` @ `f63ebaab…` as the **consolidation base** (the merge itself, not this commit), PR #5 merged and **PR #4 closed, never merged**, the four preserved SHAs (`f00f85e6`, `02b74996`, `112f1160`, `59fdacf9`), `GOVERNANCE_CHECKPOINT.md` as the live register for `T-02` / `O-*` (via [DECISIONS.md](DECISIONS.md) §20), `T-02` recorded **ACCEPTED / ASSIGNED-ONLY** rather than unresolved, the F0 / 2026-09-19 snapshot and the pre-M1 archive marked historical, and **Laravel/backend NOT STARTED and NOT AUTHORIZED**; its own §2 rows advanced to `Working branch` = `main` | The **first documentation checkpoint after the consolidation merge**, which could not name itself (no entry may carry its own SHA) — **named by this D2 pass**, which is the next one; carries no product behaviour, closes no open item, and authorizes nothing. The consolidation event itself is [recorded above](#consolidation--2026-09-20--f0f10--governance-merged-into-main-pr-5) as a ledger event, **not** as this checkpoint | ⏳ **not yet** — committed locally in this clone; remote `main` is still the consolidation merge `f63ebaab…` |
...
**named by the commit after it** — and that second commit, `8131c5cd49c35a7163543082475a9635914aba40`,
is the **nineteenth**: the M6 **coverage matrix** in [PROJECT_STATE.md](PROJECT_STATE.md) §4, where
every CP1–CP4 requirement is answered one by one as PASS / PARTIAL / DEFERRED / OUT OF SCOPE / NOT
VERIFIED. It could not name itself either, so this ledger names it here — and it is the commit M7 was
built on.

**M7's chain is registered in the M7 section above and in the milestone table**, and is listed here so
that `git log` shows nothing unexplained: four product commits —
`0d9fc01079039548cf0ffe3c80c5bf2e88056a9c` (relation plumbing — 4 files, 1073/0),
`5726435506bd74748f6165819f3c80e30f359b80` (student relations — 3 files, 899/85),
`ede3ad6a5e26b1cdfc514962b57ac1302309968c` (teacher relations — 3 files, 1330/134) and
`f1ec0ddde783aec14d6429ac2457f085f851ad9a` (class relations and navigation counts — 6 files,
1606/168) — followed by its documentation reconciliation, `e7a6d72e561bdbb0d22280e4617141281b62d8b5`,
which **could not name itself** and is therefore registered by the documentation commit after it — the
one carrying this paragraph, which also names it in [PROJECT_STATE.md](PROJECT_STATE.md) §2 and in the
table above. **`e7a6d72` is the twentieth documentation checkpoint by the definition above: documents
only, no product source, no test and no dependency** (the commit carrying this paragraph is the
twenty-first, and is registered the same way by the one after it). None of the four product commits advances the
...
checkpoint it is recorded against. `8131c5cd49c35a7163543082475a9635914aba40` is nevertheless M7's
**effective safe rollback boundary**, for the reason in the M7 section above.

**Class Compensation P1's chain is registered in the workstream section above**, and is listed here so
that `git log` shows nothing unexplained: **one product commit**,
`a21311d7e32c82e3a46b1581c94f6b3478bf646c` (*feat(compensation): add the make-up obligation domain* — 18
files, 2 976 insertions / 0 deletions; 69 tests in five new files; **no** file under
`src/domains/scheduling/`, `src/domains/attendance/` or `docs/`), followed by its documentation
reconciliation `2600667073b874d2cabb42c1e0b2f5f27234fb55` — the **twenty-first** documentation checkpoint
by the definition above (*docs(compensation): reconcile the P1 record and register D18/D19*; documents
only — the five `docs/engineering/` documents and the compensation README; no product source, no test,
no dependency), which registered P1's product commit and decided **D18**/**D19** and, like every
docs-only commit before it, could not name itself. **This commit names it.**

**P1 was not the end of the chain, and the three product commits after it are registered here for the
same reason** — so that `git log` shows nothing unexplained. **`b7a97b325741b1129f6d50a7d067d1aa6459597c`**
(*fix(compensation): enforce uniqueness and write authorization*) added MF-1's lifetime-pair re-check
immediately before the write and MF-2's `schedule.write` gate on all three verbs, and brought
`authorization.test.ts`; the domain's current total is **121 tests in eight files**, up from the 69 P1
shipped with, and the growth is enumerated in [PROJECT_STATE.md](PROJECT_STATE.md) §6.
**`cd91ed6a75a9c053b60e4e9dbbc10e06cf89a291`** (*feat(compensation): finalize C-1.1 lineage
invariants*) carries **both C-1 and C-1.1** in one commit: at most one live make-up per obligation,
refused inside one serialized section shared by `schedule` and `complete` (C-1), read through the
booking's reschedule **lineage** so a moved make-up stays `scheduled`, still refuses a second booking
and discharges against the session it was moved to (C-1.1), with the ledger never re-pointed.
**`07f89db779ca4017dbbb22db1ac7624b18fb95be`** (*feat(compensation): finalize C-2 enrollment
eligibility*) adds **D20**: the frozen student's roster state at the make-up's date is disclosed per
booking (`currentAttempt.studentOnRoster`, derived on every read, never stored) and is **never** a
booking or completion gate, while attendance keeps the refusal that owns that rule. Together those
packages carried **ten mutation checks** — C-1's three, C-1.1's five and C-2's two — each restored
byte-identically with a pre- and post-run sha256 comparison; they are recorded in the packages' own
change reports and summarised in [DECISIONS.md](DECISIONS.md) **D18**, not re-measured by this
documentation pass.

Like every workstream these advance no phase-checkpoint row — the row stays at Phase 2 — and P1's own
effective safe rollback boundary is `e7a6d72e561bdbb0d22280e4617141281b62d8b5`.
...
**`9fcf00891d07db18c718296ccaddc79919c793c1` is the twenty-fourth documentation checkpoint** and the one
that names the paragraph above (*docs(compensation): close I21 and register the S-6 boundary gate* — four
`docs/engineering/` documents, 158 insertions / 37 deletions; no product source, no test, no dependency,
and the chain commit `bf8bd911` registered inside it). It could not name itself either: **the commit
carrying this paragraph is the twenty-fifth** — the documentation pass that records **D2**, the shipped
default name, in [DECISIONS.md](DECISIONS.md) §19 — and the next documentation commit registers it,
exactly as this one registers `9fcf008`, `d18d3e3`, `2600667` and `f0bdecd`.

**`3acb5f3be275a67726376adbfed3f3fdf2f1282b` is the twenty-fifth documentation checkpoint** and the one
that names the paragraph above (*docs(governance): record decision D2 before M8* — four
`docs/engineering/` documents, 63 insertions / 32 deletions; no product source, no test, no dependency,
and the **D2** record with the shipped default name «آموزشگاه موسیقی پارسیان» inside it). It could not
name itself either: the commit that carried its paragraph was the twenty-sixth, which is also the
documentation checkpoint **the owner authorized M8 against**.

**`a039ab0e2d1f0d339b91b23c6dffc3b0a4cc5046` is the twenty-sixth documentation checkpoint** and the one
that names the paragraph above (*docs(m8): reconcile the M8 base/rollback boundary and register
checkpoint 25* — three `docs/engineering/` documents, 32 insertions / 15 deletions; no product source, no
test, no dependency, no build): M8's checkpoint paragraph was corrected so the boundary it names is the
twenty-fourth checkpoint `9fcf008` rather than M7's superseded reconciliation `e7a6d72`, whose M7
*product* record nevertheless stands, and `3acb5f3` was registered as the twenty-fifth. It is the commit
**M8 was built on**, and it could not name itself either: **the commit carrying this paragraph is the
twenty-seventh** — the documentation pass that closes M8 — and the next documentation commit registers
it, exactly as this one registers `a039ab0`, `3acb5f3`, `9fcf008`, `d18d3e3`, `2600667` and `f0bdecd`.

**M8's chain is registered here and in the milestone table**, so that `git log` shows nothing
unexplained: one product commit,
`735617d0324a8f4ba2e243846eedf069d389751a` (*feat(m8): render the persisted branding in the shell, the
login screen and the tokens* — 6 files, 360 insertions / 26 deletions, of which the new
`src/domains/branding/__tests__/brandingApplication.test.tsx` is 277 lines and 15 cases), **explicitly
authorized by the owner on 2026-09-16** and built on `a039ab0e2d1f0d339b91b23c6dffc3b0a4cc5046` — the
checkpoint immediately above. It carries its own documentation reconciliation (M8's base and effective
safe rollback boundary is `a039ab0`; the **D2** record `3acb5f3` sits immediately behind it and is the
checkpoint the owner authorized against; `9fcf008` remains the twenty-fourth; `e7a6d72` stays
superseded and was never the implementation parent) **and it does not name itself**. **M8's effective
safe rollback boundary is therefore `a039ab0`** — rolling back to it drops M8 entirely and keeps M7, the
Class Compensation workstream and its boundary gate — and its measured evidence is
[PROJECT_STATE.md](PROJECT_STATE.md) §4 → "M8 validation".

**M8's documentation closure is registered here because a commit cannot name itself**:
`c186baaa1dda39e274369ed0387094822b00f0ae` (*docs(m8): reconcile the branding phase record and
register the milestone*) is **the twenty-seventh documentation checkpoint** by the definition above —
documents only, four `docs/engineering/` documents, no product source, no test, no dependency, no gate —
and it is the commit **M9 was built on**. The commit carrying *this* paragraph is the twenty-eighth and
is registered by whichever documentation commit comes next, exactly as this one registers `c186baa`.
*(Done: the D5 record — the documentation commit that came next — registered `3a16549` as the
twenty-eighth in the ledger above.)*

**The D7/D8/D9 decision record is registered here**, so that `git log` shows nothing unexplained:
one documents-only commit (the **thirty-first** documentation checkpoint by the same count, which cannot
name itself and is registered by whichever documentation commit comes next, exactly as this passage
names `c16890f` as the **thirtieth** above): the three decisions that gate M11 — **D7** (dependency-free
accessibility assertions), **D8** (the api-hybrid disclosure behaviour) and **D9** (the
measurement-derived bundle-budget shape, on the recorded baseline of `npm run build` at `c16890f`) —
were recorded in [DECISIONS.md](DECISIONS.md) §19, with the register rows flipped and the browser-QA
environment disposition recorded in [PROJECT_STATE.md](PROJECT_STATE.md) §5. It carries no product
source, no test, no gate, **no numeric end-figure invented ahead of M11's own measurement step**, and
**it did not authorize M11** — the owner authorized the milestone
afterwards, on 2026-09-17, and the milestone row above now records it ✅ **COMPLETE**.

**M11's chain is registered here and in the milestone table**, so that `git log` shows nothing
unexplained: four product commits on top of the D7/D8/D9 decision record `3ed0bfe` —
`3eed4f190b6538ad62bf35d7b0c9fa5afc310acc` (*perf(m11): I6 — code-split views, split vendor chunk,
prune dead fonts*: route-level lazy mounting in two workspace groups — academic ≈49.8 kB gz and
operations ≈46.2 kB gz — a React-dependent **vendor** chunk (60.1 kB gz), `chunkSizeWarningLimit`
deliberately left at Vite's default of 500 kB, the provably unused Vazirmatn **300/800** weight
imports removed, `routeProtection` passing against the lazily mounted views),
`24998d8330a95510cd969c872edb28ddcc59ef16` (*test(m11): encode the D9 bundle budget gate on measured
values* — the gate pins the current measured state inside the D9 shape from DECISIONS §19:
entry < 273 791 B (measured 108 207 B), total ≤ +5% cap 305 805 B on the recorded baseline, max
single chunk < 400 000 B, weights 300/800 absent, the split real — measurement-anchored, never
backfilled from the baseline),
`53794d57a7375c8345f76adec9248f5da09d7a92` (*feat(m11): D8 — declare demo-backed domains in api
mode*: one `DEMO_SERVED_DOMAINS` enumeration over the eleven demo-in-both-modes getters at the
registry's composition root, one persistent non-dismissable `DemoBackedNotice` rendered in api mode
and absent in demo mode, wording derived from the registry seam and making no backend health or
ping claim, **6/6** including the stale-enumeration tripwire) and
`f7eb86762dc11f7d5dd3c7218e7c697fad29488e` (*test(m11): D7 — dependency-free accessibility gate* —
14 cases: accessible identity by role/Persian name, focus-trap entry/loop/Escape/return, the
command-palette keyboard contract, `prefers-reduced-motion` and `lang="fa" dir="rtl"`, stable
across **three** consecutive runs). **Starting SHA = `3ed0bfe`; final implementation HEAD =
`f7eb867`** — an implementation-report prose line that called `f7eb867` the "starting SHA" is
**corrected** here; the commits themselves were never amended. Implemented on
arena/01a0aa83-parsian-music-dashboard-opus and **explicitly authorized by the owner** after the
decision record; parent of the chain exactly `3ed0bfe4ac928451a4bddf9de3a34f7be681c33f`. The final
D9 measurements are recorded in the milestone table and in
[PROJECT_STATE.md](PROJECT_STATE.md) §4 → "M11 validation" (total gzip 298 249 B within the +5%
cap; every chunk below the 400 000 B ceiling; the >500 kB raw warning cleared by **real splitting,
not** by raising the threshold). Finance/Reports (**I2**), `docs/architecture/demo-data.md`
(**L7**), the backend/CSP/persistence plane and the M10 boundary are untouched, no dependency was
added or changed, and M10's chain remains intact. The commit carrying *this* paragraph is the
**thirty-second** documentation checkpoint by the same count, which cannot name itself and is
registered by whichever documentation commit comes next, exactly as this passage registers
`3ed0bfe` as the **thirty-first** above. **M11's effective safe rollback boundary is therefore
`3ed0bfe`** — rolling back to it drops M11 entirely and keeps the D7/D8/D9 record, M10 and
everything before — and L4 stays **NOT VERIFIED** (no browser tooling exists or was installed; the
external-QA disposition in PROJECT_STATE.md §5 stands). Nothing in this registration closes
L2, L7, I2, I19 or any other recorded residual, and no milestone beyond M11 exists in the
specification — the next phase is **NOT STARTED and NOT AUTHORIZED**.

**M10's chain is registered here and in the milestone table**, so that `git log` shows nothing
unexplained: one product commit,
`e7a64b7777be36ddd97fc3337898fad794118e5b` (*fix(reliability): M10 — dissolve the fixture boundary, no
architectural sociology* — 82 files, 2 122 insertions / 1 942 deletions: the two design-time fixture
modules dissolved, twelve new owner modules, one new 13-check boundary gate, sixteen fixture-era gates
mechanically re-derived to the same invariants against relocated owners), implemented on
arena/01a0aa83-parsian-music-dashboard-opus — **parent exactly** `98af31b5b04022362f0bc95ca99c426f2d7c8446`,
the D5 finalization record registered above — and **explicitly authorized by the owner** after the D5
finalization. It carries no document and does not name itself. The commit carrying *this* paragraph is the
**thirtieth** documentation checkpoint by the same count, which cannot name itself and is registered by whichever
documentation commit comes next, exactly as this passage registers `98af31b` as the twenty-ninth.
**M10's effective safe rollback boundary is therefore `98af31b`** — rolling back to it drops M10 entirely
and keeps the D5 record, M9, M8 and everything before — and its measured evidence is
[PROJECT_STATE.md](PROJECT_STATE.md) §4 → "M10 validation". **Branch-name errata (documented, not
repaired):** an earlier note designated arena/01a0a9b6-parsian-music-dashboard-opus as the implementation
branch; that ref **does not exist** in this repository (verified against origin and every local ref), it
was never touched, and no rename, merge, cherry-pick, reset, rebase, force-push or other branch repair was
performed. **What M10 deliberately did not do:** build a Finance or Reports domain — **I2 remains
deferred** and the two surfaces carry their deferral explicitly; build any free-slot search system;
change `BrandingSettings` persistence or schema; touch backend, or alter protected **F2/I8** semantics
(zero diff under those domains and the persistence/backup plane). That is a recorded outcome, not a
Finance/Reports implementation.

**M9's chain is registered here and in the milestone table**, so that `git log` shows nothing
unexplained: one product commit,
`8d34eb3d1cd639cffc794596250c897b5ed4b6b3` (*feat(m9): derive every dashboard insight from stored
records, guarding the empty series first* — 14 files, 3 025 insertions / 328 deletions: eight modified
files and six new ones, four of which are test files carrying **61 new cases** in
`src/components/ds/__tests__/seriesGuards.test.tsx` (7), `src/domains/shared/__tests__/dashboardInsights.test.ts`
(35), `src/components/panels/__tests__/panelsEmpty.test.tsx` (12) and
`src/views/__tests__/dashboardInsightsLive.test.tsx` (7)), **explicitly authorized by the owner on
2026-09-16** and built on `c186baaa1dda39e274369ed0387094822b00f0ae` — the documentation checkpoint
immediately above. It carries no document and does not name itself. **M9's effective safe rollback
boundary is therefore `c186baa`** — rolling back to it drops M9 entirely and keeps M8, the Class
Compensation workstream and its boundary gate — and its measured evidence is
[PROJECT_STATE.md](PROJECT_STATE.md) §4 → "M9 validation". **The one thing M9 did not do is build the
thing it found missing:** no Finance domain, no `invoices`/`payments` repository and no fabricated
revenue figure — the revenue chart was **removed** and the money slot now shows the receivables the
student records carry, because collected revenue has no authoritative seam and **D6**/**I2** keep those
domains *planned*. That is a recorded outcome, not a Finance/Reports implementation.
