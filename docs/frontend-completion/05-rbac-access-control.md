...
| T-02 | teacher | read | unassigned student (not enrolled in any class teacher teaches) | org (other student) | **OPEN** — CURRENT OBSERVED BEHAVIOR: **DENIED** — PRODUCT DECISION: **OPEN** — must be resolved before RBAC implementation reaches this scope — do not silently select ALLOW or DENY | CURRENT OBSERVED BEHAVIOR: **DENIED** — frontend currently shows no filter? Actually historical note: teacher had students.read org-wide but current observed in authorization contract review: DENIED — product decision remains OPEN — O-01 scope decision — must be resolved before implementation — document as CURRENT OBSERVED DENIED, PRODUCT DECISION OPEN, explicitly OPEN, not silently ALLOW/DENY | Frontend: currently **no filter shows DENIED behavior** — if scope enforced later, must hide unassigned — decision OPEN — frontend currently behaves as DENIED? Actually observed DENIED — so frontend shows DENIED — but product decision OPEN — must be resolved | Backend: if scope enforced, must **403** if DENIED, but decision is **OPEN** — so backend must **not** silently choose ALLOW/DENY — must be resolved before RBAC implementation reaches this scope — record as OPEN, do not silently select — PRODUCT DECISION OPEN |
