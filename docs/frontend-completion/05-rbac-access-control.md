...
| # | Actor | Action | Resource | Scope | Allowed? | Reason | Frontend guard | Future backend enforcement |
|---|---|---|---|---|---|---|---|---|
| T-01 | teacher | read | assigned student (enrolled in class teacher teaches) | assigned (Enrollment active scoped to date where class.teacherId = teacherId) | ALLOWED | teacher has students.read + assigned scope via enrollment → class → teacher | Frontend: Students list currently org-wide (gap) — should filter to assigned via enrollment join, but currently org-wide — needs decision T-02 — for now org-wide with note, future filter via useClasses({teacherId}) + enrollment | Server must enforce: org_id + permission students.read + scope assigned = enrollment exists for class where teacherId = actor's linked teacherId and enrollment active — 403 if not assigned (if assigned scope enforced) — currently org-wide read is allowed per current matrix, but assigned scope is desired smallest — decision needed, document both current org-wide and desired assigned |
| T-02 | teacher | read | unassigned student (not enrolled in any class teacher teaches) | org (other student) | **STATUS: OPEN — EVIDENCE CONFLICTING — REQUIRES EMPIRICAL RE-VERIFICATION — PRODUCT DECISION OPEN — do NOT silently select ALLOW or DENY — empirical verification deferred to F2** — CURRENT OBSERVED BEHAVIOR: **DENIED** (observed in authorization contract review) — HISTORICAL EVIDENCE: **ALLOWED** org-wide per earlier matrix / READ_ONLY_CORE teacher had students.read org-wide — **EVIDENCE CONFLICTING** — must be resolved before RBAC implementation reaches this scope — preserve both DENIED/ALLOWED evidences, no silent normalize, no final ALLOW/DENY — empirical to F2 | **EVIDENCE 1 — DENIED:** frontend currently shows no filter? Actually historical note: teacher had students.read org-wide but current observed in authorization contract review: DENIED — product decision remains OPEN — T-02 assigned-scope decision — must be resolved before implementation — document as CURRENT OBSERVED DENIED, PRODUCT DECISION OPEN, explicitly OPEN, not silently ALLOW/DENY — **EVIDENCE 2 — ALLOWED:** earlier org-wide matrix READ_ONLY_CORE gave teacher students.read org-wide, teacher saw all students org-wide — gap assigned scope decision T-02 — both evidences preserved — **STATUS OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION** — do NOT normalize — empirical verification to F2 | Frontend: currently **no filter shows DENIED behavior** — if scope enforced later, must hide unassigned — decision OPEN — frontend currently behaves as DENIED? Actually observed DENIED — so frontend shows DENIED — but product decision OPEN — must be resolved — **EVIDENCE CONFLICTING preserved: DENIED observed now, ALLOWED observed historically org-wide** — STATUS OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION — empirical to F2 | Backend: if scope enforced, must **403** if DENIED, but decision is **OPEN** — so backend must **not** silently choose ALLOW/DENY — must be resolved before RBAC implementation reaches this scope — record as OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION — preserve DENIED/ALLOWED evidences — no silent normalize — empirical verification deferred to F2 — PRODUCT DECISION OPEN — STATUS OPEN EVIDENCE CONFLICTING |
...
- Enrollment canonical Student↔Class edge, but does not restrict read scope — teacher sees all students org-wide currently, not just assigned via enrollment — gap assigned scope decision **T-02** (not O-01; O-01 student level per-program DECIDED 2026-09-21)
...
- Testable cases S-01..S-04, T-01..T-04, ST-01..ST-02, M-01..M-03, A-01, X-01..X-11 covered — 11 required cases explicitly: student→own profile ALLOWED S-01, student→another student's profile DENIED S-02, student→eligible resources ALLOWED S-03, student→higher-level locked DENIED S-04, teacher→assigned student ALLOWED T-01, teacher→unassigned student **STATUS OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION — EVIDENCE 1 DENIED observed now + EVIDENCE 2 ALLOWED observed historically org-wide — preserve both — no silent normalize — empirical to F2** T-02 must be resolved before RBAC implementation reaches this scope, teacher→attendance for assigned session ALLOWED T-03, teacher→compensation write DENIED T-04, staff→operational student/class/session actions ALLOWED ST-01, manager→organization-wide operations ALLOWED M-01, administrator→organization administration ALLOWED A-01 — RBAC T-02 STATUS OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION — DENIED + ALLOWED evidences preserved — NEXT ACTION exactly ONE F1
...
- Teacher currently sees all students org-wide — if assigned scope desired, need enrollment-based filtering — decision needed **T-02**, not silently assumed — documented as **EVIDENCE CONFLICTING**: historical ALLOWED org-wide (READ_ONLY_CORE teacher students.read) + current observed DENIED per authorization contract review — STATUS OPEN EVIDENCE CONFLICTING REQUIRES EMPIRICAL RE-VERIFICATION — preserve both — empirical to F2 — no silent normalize (O-01 student level per-program is DECIDED 2026-09-21 and is separate from T-02 teacher scope)
