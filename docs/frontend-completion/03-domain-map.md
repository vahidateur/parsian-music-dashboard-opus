...
> One owner per rule, one source per concept. No unjustified relations. Active branch `arena/frontend-completion-spec` SHA `e57bf19` = `e57bf1922e4ca3b80796715028445cfe8a077c9c` (previous `c244fec` preserved as historical) spec `docs/frontend-completion/` v14 @ `e57bf19` PR #4 F0 COMPLETE + corrections F1 NEXT — current_sha = e57bf19 exactly, c244fec preserved only as previous — classification correction Telegram backup/student access/Bale/Mobile = REQUIRED PRODUCT CAPABILITY IMPLEMENTATION deferred until backend/integration layer exists — learning scope OPEN global vs per-program vs per-instrument do not invent O-01 — canonical Level N=>1..N owner learning/eligibility.ts — RBAC T-02 OPEN CURRENT OBSERVED DENIED PRODUCT DECISION OPEN — NEXT ACTION exactly ONE F1
